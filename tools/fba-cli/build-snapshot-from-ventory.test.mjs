...
test("Transit rein: wh=0, onway=63 -> threePLUnits=63", () => {
...
test("max, kein Doppel: wh=504, onway=504 -> threePLUnits=504 (nicht 1008)", () => {
  const it = mapVentoryRowToItem(
    { wh_pcs_left: 504, fba_pcs_on_the_way: 504, InStockSupplyQuantity: 0, afn_reserved_quantity: 0 },
    "TEST-FRAMEBAG-EDGE",
  );
  assert.equal(it.threePLUnits, 504);
  assert.notEqual(it.threePLUnits, 1008);
});

test("Lager dominiert: wh=1300, onway=19 -> threePLUnits=1300", () => {
  const it = mapVentoryRowToItem(
