...
`)}function J(t){var s;if((s=navigator.clipboard)!=null&&s.writeText)return navigator.clipboard.writeText(t);const e=document.createElement("textarea");e.value=t,e.style.position="fixed",e.style.opacity="0",document.body.appendChild(e),e.focus(),e.select();try{document.execCommand("copy")}finally{document.body.removeChild(e)}return Promise.resolve()}function X(t,e,s){var A,F;const c=(A=e==null?void 0:e.details)==null?void 0:A[s];if(!c)return;const r=_[s]||s,m=B(e.month),i=Array.isArray(c.items)?c.items:[],S=Q[s]||[{label:"Posten",getValue:o=>o.label,align:"left"},{label:"Betrag",getValue:o=>n(o.amount),align:"num"}],u=document.createElement("div");u.className="po-modal-backdrop vat-detail-modal",u.setAttribute("role","dialog"),u.setAttribute("aria-modal","true");const v=document.createElement("div");v.className="po-modal vat-detail-modal-frame",v.innerHTML=`
  ...
  `;const g=document.createElement("div");if(g.className="po-modal-body",c.notes){const o=document.createElement("p");o.className="muted small vat-detail-notes",o.textContent=c.notes,g.appendChild(o)}const E=document.createElement("div");E.className="vat-detail-controls";let a="",l=i;const d=document.createElement("input");d.type="search",d.placeholder="Suche in Details",d.className="vat-detail-search",i.length>50&&E.appendChild(d);const f=document.createElement("div");f.className="muted small vat-detail-limit",i.length>50&&(f.textContent="Es werden nur die ersten 50 Zeilen angezeigt (Nutze Suche oder CSV kopieren).");const p=document.createElement("div");p.className="vat-detail-table-wrap";const x=document.createElement("table");x.className="table-compact vat-detail-table",x.innerHTML=`
    <thead>
      <tr>
        ${S.map(o=>`<th class="${o.align==="num"?"num":""}">${o.label}</th>`).join("")}
      </tr>
    </thead>
    <tbody></tbody>
    <tfoot>
      <tr>
        <td class="vat-detail-sum" colspan="${Math.max(1,S.length-1)}">Summe</td>
        <td class="num vat-detail-sum">${n(c.total??0)}</td>
      </tr>
    </tfoot>
  `,p.appendChild(x);const h=document.createElement("p");h.className="muted small vat-detail-empty",h.textContent="Keine Details verfügbar.";function b(){if(!a)l=i;else{const o=a.toLowerCase();l=i.filter(C=>{var L;return[C.label,C.sublabel,C.date,(L=C.meta)==null?void 0:L.sourceNumber].filter(Boolean).map(k=>String(k).toLowerCase()).join(" ").includes(o)})}}function y(){b();const o=x.querySelector("tbody");if(o.innerHTML="",!l.length){p.replaceWith(h);return}p.isConnected||h.replaceWith(p),(i.length>50&&!a?l.slice(0,50):l).forEach(z=>{const L=document.createElement("tr");S.forEach(k=>{const I=document.createElement("td");k.align==="num"&&(I.className="num"),I.textContent=k.getValue(z),L.appendChild(I)}),o.appendChild(L)})}d.addEventListener("input",o=>{a=o.target.value||"",y()});const V=document.createElement("footer");V.className="po-modal-actions";const $=document.createElement("button");$.type="button",$.className="btn secondary",$.textContent="CSV kopieren",$.addEventListener("click",()=>{b();const o=W(S,l);J(o)});const D=document.createElement("button");D.type="button",D.className="btn",D.textContent="Schließen",V.append($,D),E.childNodes.length&&g.appendChild(E),f.textContent&&g.appendChild(f),g.appendChild(i.length?p:h),v.appendChild(g),v.appendChild(V),u.appendChild(v),document.body.appendChild(u);function N(){u.remove()}u.addEventListener("click",o=>{o.target===u&&N()}),(F=v.querySelector("[data-close]"))==null||F.addEventListener("click",N),D.addEventListener("click",N),y()}function Y(t){const e=document.createElement("div");return e.className="vat-preview-kpis",e.innerHTML=`
  ...
  `,e}function tt(t,e){var i,S,u,v,g,E;const s=document.createElement("div");s.className="panel vat-panel",s.innerHTML=`
  ...
  `;const c=s.querySelector(".vat-body");e.rows.forEach((a,l)=>{const d=e.monthConfig[a.month],f=B(a.month),p=a.payable<0?"is-negative":"",x=l===0?"disabled":"",h=document.createElement("tr");h.className="vat-preview-row",h.dataset.month=a.month,h.innerHTML=`
    ...
    `,c.appendChild(h),c.appendChild(b)});const r=K(e),m=Y(e);(i=s.querySelector(".panel-header"))==null||i.after(r),r.after(m),(S=r.querySelector("#vat-eust-lag"))==null||S.addEventListener("change",a=>{w({eustLagMonths:Number(a.target.value)||0})}),(u=r.querySelector("#vat-de-share"))==null||u.addEventListener("change",a=>{w({deShareDefault:M(a.target.value)})}),(v=r.querySelector("#vat-fee-rate"))==null||v.addEventListener("change",a=>{w({feeRateDefault:M(a.target.value)})}),(g=r.querySelector("#vat-fix-input"))==null||g.addEventListener("change",a=>{w({fixInputDefault:T(a.target.value)})}),(E=r.querySelector("#vat-reset"))==null||E.addEventListener("click",()=>{q()}),c.addEventListener("change",a=>{const l=a.target,d=l.getAttribute("data-field"),f=l.getAttribute("data-month");if(!d||!f)return;const p=d==="fixInputVat"?T(l.value):M(l.value);U(f,{[d]:p})}),c.addEventListener("click",a=>{const l=a.target.closest("[data-toggle]");if(l){const b=l.getAttribute("data-toggle"),y=c.querySelector(`[data-details="${b}"]`),V=l.getAttribute("aria-expanded")==="true";y&&(y.hidden=V,l.setAttribute("aria-expanded",String(!V)),l.textContent=V?"Bearbeiten":"Schließen");return}const d=a.target.closest(".vat-detail-trigger");if(d){const b=d.getAttribute("data-month"),y=d.getAttribute("data-detail-key"),V=e.rows.find($=>$.month===b);V&&y&&X(t,V,y);return}const f=a.target.closest("[data-copy-prev]");if(!f)return;const p=f.getAttribute("data-copy-prev"),x=e.months,h=x.indexOf(p);if(h>0){const b=x[h-1],y=e.monthConfig[b];U(p,{deShare:y.deShare,feeRateOfGross:y.feeRateOfGross,fixInputVat:y.fixInputVat})}}),t.appendChild(s)}function at(t){const e=t;e.innerHTML="";function s(){const r=R(),m=O(r),i={};m.months.forEach(u=>{var v,g,E,a,l,d;i[u]={deShare:Number(((g=(v=r.vatPreviewMonths)==null?void 0:v[u])==null?void 0:g.deShare)??r.settings.vatPreview.deShareDefault??.8),feeRateOfGross:Number(((a=(E=r.vatPreviewMonths)==null?void 0:E[u])==null?void 0:a.feeRateOfGross)??r.settings.vatPreview.feeRateDefault??.38),fixInputVat:T(((d=(l=r.vatPreviewMonths)==null?void 0:l[u])==null?void 0:d.fixInputVat)??r.settings.vatPreview.fixInputDefault??0)}}),m.settings=r.settings.vatPreview,m.monthConfig=i,m.months=m.months,e.innerHTML="",tt(e,m);const S=window.__routeQuery||{};if(S.month){const u=e.querySelector(`.vat-preview-row[data-month="${S.month}"]`);u&&(u.classList.add("row-focus"),u.scrollIntoView({block:"center",behavior:"smooth"}),window.__routeQuery={})}}return s(),{cleanup:P(()=>s())}}export{at as default};
