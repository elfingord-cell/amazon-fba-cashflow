import{a as P,n as O,h as R,p as T,U as w,V as q,W as U,X as G}from"./index-12pF4-sN.js";const H=new Intl.DateTimeFormat("de-DE",{month:"short",year:"numeric"}),j=new Intl.DateTimeFormat("de-DE",{day:"2-digit",month:"2-digit",year:"numeric"}),_={deBrutto:"DE-Brutto",outputUst:"Output-USt",vstFees:"VSt Fees",fixkostenVst:"Fixkosten-VSt",eustErstattung:"EUSt-Erstattung",zahllast:"Zahllast"},W={deBrutto:[{label:"SKU / Alias",getValue:t=>[t.label,t.sublabel].filter(Boolean).join(" – "),align:"left"},{label:"Units",getValue:t=>{var e;return((e=t.meta)==null?void 0:e.units)!=null?Z(t.meta.units):"–"},align:"num"},{label:"Preis",getValue:t=>{var e;return((e=t.meta)==null?void 0:e.price)!=null?n(t.meta.price):"–"},align:"num"},{label:"Umsatzbeitrag",getValue:t=>n(t.amount),align:"num"}],outputUst:[{label:"Posten",getValue:t=>t.label,align:"left"},{label:"Betrag",getValue:t=>n(t.amount),align:"num"}],vstFees:[{label:"Quelle/Typ",getValue:t=>t.label,align:"left"},{label:"Basis",getValue:t=>t.sublabel||"–",align:"left"},{label:"VSt Betrag",getValue:t=>n(t.amount),align:"num"}],fixkostenVst:[{label:"Name",getValue:t=>t.label,align:"left"},{label:"VSt Anteil",getValue:t=>n(t.amount),align:"num"}],eustErstattung:[{label:"PO/FO",getValue:t=>t.label||"–",align:"left"},{label:"Event",getValue:t=>t.sublabel||"–",align:"left"},{label:"Datum",getValue:t=>t.date?j.format(new Date(t.date)):"–",align:"left"},{label:"Betrag",getValue:t=>n(t.amount),align:"num"}],zahllast:[{label:"Komponente",getValue:t=>t.label,align:"left"},{label:"Betrag",getValue:t=>n(t.amount),align:"num"}]};function n(t){return G(Number(t||0))}function Z(t,e=0){return Number.isFinite(Number(t))?Number(t).toLocaleString("de-DE",{minimumFractionDigits:e,maximumFractionDigits:e}):"–"}function B(t){const[e,s]=t.split("-").map(Number);return H.format(new Date(e,s-1,1))}function M(t){const e=Number(String(t??"").replace(",","."));return Number.isFinite(e)?e:0}function K(t){const e=document.createElement("div");return e.className="vat-preview-toolbar",e.innerHTML=`
  ...
  `,e}function Q(t,e){const s=m=>{const o=String(m??"");return o.includes(";")||o.includes(`
`)||o.includes('"')?`"${o.replace(/"/g,'""')}"`:o},c=t.map(m=>s(m.label)).join(";"),r=e.map(m=>t.map(o=>s(o.getValue(m))).join(";"));return[c,...r].join(`
`)}function X(t){var s;if((s=navigator.clipboard)!=null&&s.writeText)return navigator.clipboard.writeText(t);const e=document.createElement("textarea");e.value=t,e.style.position="fixed",e.style.opacity="0",document.body.appendChild(e),e.focus(),e.select();try{document.execCommand("copy")}finally{document.body.removeChild(e)}return Promise.resolve()}function J(t,e,s){var A,F;const c=(A=e==null?void 0:e.details)==null?void 0:A[s];if(!c)return;const r=_[s]||s,m=B(e.month),o=Array.isArray(c.items)?c.items:[],E=W[s]||[{label:"Posten",getValue:i=>i.label,align:"left"},{label:"Betrag",getValue:i=>n(i.amount),align:"num"}],u=document.createElement("div");u.className="po-modal-backdrop vat-detail-modal",u.setAttribute("role","dialog"),u.setAttribute("aria-modal","true");const v=document.createElement("div");v.className="po-modal vat-detail-modal-frame",v.innerHTML=`
  ...
  `;const g=document.createElement("div");if(g.className="po-modal-body",c.notes){const i=document.createElement("p");i.className="muted small vat-detail-notes",i.textContent=c.notes,g.appendChild(i)}const S=document.createElement("div");S.className="vat-detail-controls";let a="",l=o;const d=document.createElement("input");d.type="search",d.placeholder="Suche in Details",d.className="vat-detail-search",o.length>50&&S.appendChild(d);const f=document.createElement("div");f.className="muted small vat-detail-limit",o.length>50&&(f.textContent="Es werden nur die ersten 50 Zeilen angezeigt (Nutze Suche oder CSV kopieren).");const p=document.createElement("div");p.className="vat-detail-table-wrap";const x=document.createElement("table");x.className="table-compact vat-detail-table",x.innerHTML=`
    <thead>
      <tr>
        ${E.map(i=>`<th class="${i.align==="num"?"num":""}">${i.label}</th>`).join("")}
      </tr>
    </thead>
    <tbody></tbody>
    <tfoot>
  ...
  `,p.appendChild(x);const h=document.createElement("p");h.className="muted small vat-detail-empty",h.textContent="Keine Details verfügbar.";function b(){if(!a)l=o;else{const i=a.toLowerCase();l=o.filter(C=>{var L;return[C.label,C.sublabel,C.date,(L=C.meta)==null?void 0:L.sourceNumber].filter(Boolean).map(k=>String(k).toLowerCase()).join(" ").includes(i)})}}function y(){b();const i=x.querySelector("tbody");if(i.innerHTML="",!l.length){p.replaceWith(h);return}p.isConnected||h.replaceWith(p),(o.length>50&&!a?l.slice(0,50):l).forEach(z=>{const L=document.createElement("tr");E.forEach(k=>{const I=document.createElement("td");k.align==="num"&&(I.className="num"),I.textContent=k.getValue(z),L.appendChild(I)}),i.appendChild(L)})}d.addEventListener("input",i=>{a=i.target.value||"",y()});const V=document.createElement("footer");V.className="po-modal-actions";const $=document.createElement("button");$.type="button",$.className="btn secondary",$.textContent="CSV kopieren",$.addEventListener("click",()=>{b();const i=Q(E,l);X(i)});const D=document.createElement("button");D.type="button",D.className="btn",D.textContent="Schließen",V.append($,D),S.childNodes.length&&g.appendChild(S),f.textContent&&g.appendChild(f),g.appendChild(o.length?p:h),v.appendChild(g),v.appendChild(V),u.appendChild(v),document.body.appendChild(u);function N(){u.remove()}u.addEventListener("click",i=>{i.target===u&&N()}),(F=v.querySelector("[data-close]"))==null||F.addEventListener("click",N),D.addEventListener("click",N),y()}function Y(t){const e=document.createElement("div");return e.className="vat-preview-kpis",e.innerHTML=`
  ...
  `,e}function tt(t,e){var o,E,u,v,g,S;const s=document.createElement("div");s.className="panel vat-panel",s.innerHTML=`
  ...
  `;const c=s.querySelector(".vat-body");e.rows.forEach((a,l)=>{const d=e.monthConfig[a.month],f=B(a.month),p=a.payable<0?"is-negative":"",x=l===0?"disabled":"",h=document.createElement("tr");h.className="vat-preview-row",h.dataset.month=a.month,h.innerHTML=`
      ...
      <td class="vat-preview-action">
        <button type="button" class="btn ghost sm" data-toggle="${a.month}" aria-expanded="false">Bearbeiten</button>
      </td>
    ...
    `,c.appendChild(h),c.appendChild(b)});const r=K(e),m=Y(e);(o=s.querySelector(".panel-header"))==null||o.after(r),r.after(m),(E=r.querySelector("#vat-eust-lag"))==null||E.addEventListener("change",a=>{w({eustLagMonths:Number(a.target.value)||0})}),(u=r.querySelector("#vat-de-share"))==null||u.addEventListener("change",a=>{w({deShareDefault:M(a.target.value)})}),(v=r.querySelector("#vat-fee-rate"))==null||v.addEventListener("change",a=>{w({feeRateDefault:M(a.target.value)})}),(g=r.querySelector("#vat-fix-input"))==null||g.addEventListener("change",a=>{w({fixInputDefault:T(a.target.value)})}),(S=r.querySelector("#vat-reset"))==null||S.addEventListener("click",()=>{q()}),c.addEventListener("change",a=>{const l=a.target,d=l.getAttribute("data-field"),f=l.getAttribute("data-month");if(!d||!f)return;const p=d==="fixInputVat"?T(l.value):M(l.value);U(f,{[d]:p})}),c.addEventListener("click",a=>{const l=a.target.closest("[data-toggle]");if(l){const b=l.getAttribute("data-toggle"),y=c.querySelector(`[data-details="${b}"]`),V=l.getAttribute("aria-expanded")==="true";y&&(y.hidden=V,l.setAttribute("aria-expanded",String(!V)),l.textContent=V?"Bearbeiten":"Schließen");return}const d=a.target.closest(".vat-detail-trigger");if(d){const b=d.getAttribute("data-month"),y=d.getAttribute("data-detail-key"),V=e.rows.find($=>$.month===b);V&&y&&J(t,V,y);return}const f=a.target.closest("[data-copy-prev]");if(!f)return;const p=f.getAttribute("data-copy-prev"),x=e.months,h=x.indexOf(p);if(h>0){const b=x[h-1],y=e.monthConfig[b];U(p,{deShare:y.deShare,feeRateOfGross:y.feeRateOfGross,fixInputVat:y.fixInputVat})}}),t.appendChild(s)}function at(t){const e=t;e.innerHTML="";function s(){const r=O(),m=R(r),o={};m.months.forEach(u=>{var v,g,S,a,l,d;o[u]={deShare:Number(((g=(v=r.vatPreviewMonths)==null?void 0:v[u])==null?void 0:g.deShare)??r.settings.vatPreview.deShareDefault??.8),feeRateOfGross:Number(((a=(S=r.vatPreviewMonths)==null?void 0:S[u])==null?void 0:a.feeRateOfGross)??r.settings.vatPreview.feeRateDefault??.38),fixInputVat:T(((d=(l=r.vatPreviewMonths)==null?void 0:l[u])==null?void 0:d.fixInputVat)??r.settings.vatPreview.fixInputDefault??0)}}),m.settings=r.settings.vatPreview,m.monthConfig=o,m.months=m.months,e.innerHTML="",tt(e,m);const E=window.__routeQuery||{};if(E.month){const u=e.querySelector(`.vat-preview-row[data-month="${E.month}"]`);u&&(u.classList.add("row-focus"),u.scrollIntoView({block:"center",behavior:"smooth"}),window.__routeQuery={})}}return s(),{cleanup:P(()=>s())}}export{at as default};
