...
`),Ie=p.id==="balance"&&T.isActual?`<div class="balance-detail"><span>Plan: ${ae(T.plannedTotal)}</span><span>Ist: ${ae(T.actualTotal)}</span></div>`:"",he=p.nav&&!K.isEmpty,De=p.nav?encodeURIComponent(JSON.stringify({...p.nav,month:t})):"";return`
          ...
          `}).join(""),H=(()=>{if(p.id!=="balance")return'<td class="num dashboard-compare-cell muted">—</td>';const t=i.slice().reverse().find($=>{var L;return(L=p.values[$])==null?void 0:L.isActual});if(!t)return'<td class="num dashboard-compare-cell muted">—</td>';const d=p.values[t]||{};return`
        ...
        `})();return`<tr data-row-id="${E(p.id)}" class="${G}">${O}${j}${H}</tr>`}).join("");return`
  ...
  `}function yt(e){var w,F;if(!e)return"";const n=e.statusKey||"gray",a=e.status||((w=se[n])==null?void 0:w.label)||"—",o=ie(e.monthKey),s=Number(e.activeSkus||0),r=Number(e.coveredSkus||0),u=Number(e.coverageRatio||0),f=e.missingCritical||{},h=!!e.taxesActive,b=e.projectionMode==="doh"?"doh":"units",l=b==="doh"?"DOH":"Units",c=b==="doh"?"Safety DOH":"Safety Units",m=s>0&&u>=q.full,i=[{label:"Inventory Coverage ok?",description:`${r}/${s} aktive SKUs abgedeckt (${W(u)}).`,passed:m},{label:"Amazon payouts vorhanden?",description:"Amazon-Auszahlungen für den Monat sind erfasst.",passed:!f.amazonPayout},{label:"Fixkosten vorhanden?",description:"Fixkosten für den Monat sind gepflegt.",passed:!f.fixedCosts},{label:"Steuer-Config ok?",description:h?"USt-Vorschau ist konfiguriert.":"USt-Vorschau ist nicht aktiv.",passed:h?!f.taxes:!0}],g=`Schwellen: Vollständig ≥${W(q.full)}, Weitgehend ≥${W(q.wide)}, Teilweise ≥${W(q.partial)}.`,y=Array.isArray(e.todoLinks)?e.todoLinks:[],N=n!=="green"&&y.length>0?y.map(C=>`
      <li>
