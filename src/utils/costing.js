...
export function toEurFromUsd(usd, fxUsdPerEur) {
  const usdValue = toNumber(usd);
  const fxRate = toNumber(fxUsdPerEur);
  if (usdValue == null || fxRate == null || fxRate <= 0) return null;
  return usdValue / fxRate;
}

export function computeFreightPerUnitEur({ unitPriceUsd, landedCostEur, fxUsdPerEur }) {
  const missingFields = [];
  const unitUsd = toNumber(unitPriceUsd);
  const landedEur = toNumber(landedCostEur);
  const fxRate = toNumber(fxUsdPerEur);

  if (unitUsd == null || unitUsd <= 0) missingFields.push("unitPriceUsd");
  if (landedEur == null || landedEur <= 0) missingFields.push("landedCostEur");
  if (fxRate == null || fxRate <= 0) missingFields.push("fxUsdPerEur");
