...
export interface PhantomFoWorklistEntry {
  key: string;
  sku: string;
  alias: string;
  abcClass: "A" | "B" | "C";
...
export function resolvePhantomFoOrderDateIso(input: {
...
export function buildPhantomFoSuggestions(input: {
  ...
  const planningState = sharedPlanProjection.planningState;
  ...
  const pfoDecisionById = resolvePfoWorklistDecisionById(settings);

  const liveProducts = (Array.isArray(planningState.products) ? planningState.products : [])
  ...
  for (let iteration = 0; iteration < maxIterations; iteration += 1) {
    ...
    const scopedIssues = buildScopedOrderDutyIssues({
      ...
      .filter((issue) => {
        ...
        return !blockedSkuKeys.has(skuKey);
      });
  ...
  const openSuggestions = suggestions.filter((entry) => {
    return pfoDecisionById.get(entry.id)?.decision !== "fo_converted";
  });

  openSuggestions.sort(compareSuggestionPriority);
  if (maxSuggestions != null) {
    return openSuggestions.slice(0, maxSuggestions);
  }
...
export function buildPhantomFoWorklist(input: {
  state: Record<string, unknown>;
  baseMonth?: string | null;
  windowMonths?: number;
  months?: string[] | null;
  maxSuggestions?: number;
}): PhantomFoWorklistEntry[] {
  const baseMonth = normalizeMonthKey(input.baseMonth) || currentMonthKey();
  const windowMonths = Math.max(1, Math.round(Number(input.windowMonths || 6)));
  const windowEndMonth = addMonths(baseMonth, windowMonths - 1);
  const suggestions = buildPhantomFoSuggestions({
    state: input.state,
    months: input.months,
  ...
  return suggestions
    .map((entry) => {
      ...
      const includeInWindow = Boolean(orderMonth && orderMonth >= baseMonth && orderMonth <= windowEndMonth);
      if (!entry.overdue && !includeInWindow) return null;
      return {
        key: entry.id,
        sku: entry.sku,
        alias: entry.alias,
        ...
        orderMonth: orderMonth || normalizeMonthKey(entry.firstRiskMonth) || baseMonth,
        orderDateIso: resolvePhantomFoOrderDateIso(entry),
        ...
        suggestedUnits: Math.max(0, Math.round(Number(entry.suggestedUnits || 0))),
        overdue: entry.overdue === true,
      } satisfies PhantomFoWorklistEntry;
    })
    .filter((entry): entry is PhantomFoWorklistEntry => Boolean(entry))
    .sort((left, right) => {
      const leftOrder = left.orderDateIso || left.recommendedOrderDate || left.latestOrderDate || `${left.orderMonth}-01`;
      const rightOrder = right.orderDateIso || right.recommendedOrderDate || right.latestOrderDate || `${right.orderMonth}-01`;
