...
function quarterKeyForMonth(monthKey) {
  ...
  if (!normalizedMonth || quarterIndex == null) return "";
...
function quarterLabel(quarterKey) {
  ...
  if (!parsed) return String(quarterKey || "");
...
function quarterKeysFromPaymentMonths(paymentMonths) {
  return Array.from(new Set(
    uniqueMonths(paymentMonths).map((paymentMonth) => {
...
function normalizeOverrideMap(raw) {
  ...
  return Object.entries(raw).reduce((acc, [month, value]) => {
    ...
    if (!normalizedMonth) return acc;
    acc[normalizedMonth] = normalizeTaxOverride(value);
    return acc;
  }, {});
}

export function createDefaultTaxesState() {
  return {
...
export function normalizeTaxesState(input) {
...
export function expandTaxInstances(state, opts = {}) {
  ...
  TAX_TYPE_CONFIG.forEach((taxType) => {
    ...
    if (!baseAmount || !firstDueDate || firstDueIdx == null) return;
    ...
    months.forEach((month) => {
      ...
      if (currentIdx == null || currentIdx < firstDueIdx) return;
      if ((currentIdx - firstDueIdx) % 3 !== 0) return;
      if (pauseFromIdx != null && currentIdx >= pauseFromIdx) return;
  ...
  results.sort((left, right) => {
    if (left.month === right.month) {
      if (left.dueDateIso === right.dueDateIso) return String(left.label || "").localeCompare(String(right.label || ""));
...
export function expandVatTaxInstances(state, opts = {}) {
  const sourceState = state && typeof state === "object" ? state : {};
  const settings = sourceState.settings && typeof sourceState.settings === "object" ? sourceState.settings : {};
  const startMonth = normalizeMonth(opts.startMonth || settings.startMonth) || "2025-01";
  const horizon = Math.max(1, Number(opts.horizon || settings.horizonMonths || 12) || 12);
  ...
  vatPreviewState.settings.horizonMonths = Math.max(
    1,
    months.length + paymentLagMonths,
    Number(vatPreviewState.settings.horizonMonths || 0) || 0,
  );
  const preview = computeVatPreview(vatPreviewState);
...
export function buildOssQuarterPreview(state, opts = {}) {
  const sourceState = state && typeof state === "object" ? state : {};
  const settings = sourceState.settings && typeof sourceState.settings === "object" ? sourceState.settings : {};
  const startMonth = normalizeMonth(opts.startMonth || settings.startMonth) || "2025-01";
  const horizon = Math.max(1, Number(opts.horizon || settings.horizonMonths || 12) || 12);
  ...
  const quarterKeys = paymentMonths.length
    ...
    : Array.from(new Set(
      (sourceMonths.length ? sourceMonths : monthRange(startMonth, horizon))
        .map((month) => quarterKeyForMonth(month))
        .filter(Boolean),
    ));
  if (!quarterKeys.length) return [];

  const requiredSourceMonths = Array.from(new Set(
    quarterKeys.flatMap((quarterKey) => sourceMonthsForQuarter(quarterKey)),
  )).sort((left, right) => left.localeCompare(right));
  const previewState = buildPreviewStateForMonths(sourceState, requiredSourceMonths, startMonth, horizon);
  if (!previewState) return [];
  const preview = computeVatPreview(previewState);
  const rowsByMonth = new Map(
    (Array.isArray(preview?.rows) ? preview.rows : []).map((row) => [normalizeMonth(row?.month), row]),
  );
...
export function expandOssTaxInstances(state, opts = {}) {
...
export function expandMonthlyTaxCashflowInstances(state, opts = {}) {
  return [
...
export function buildMonthlyTaxSummary(instances, months = [], typeConfig = DASHBOARD_TAX_TYPE_CONFIG) {
  const summary = new Map();
  const seedMonths = Array.isArray(months) ? months : [];
  const allowedTypes = new Set(
    Array.isArray(typeConfig) && typeConfig.length
      ? typeConfig.map((entry) => String(entry?.key || "")).filter(Boolean)
      : Array.from(DASHBOARD_TAX_TYPE_KEYS),
  );
  const typeTemplate = buildTaxTypeTemplate(
    Array.isArray(typeConfig) && typeConfig.length
      ? typeConfig
  ...
  (Array.isArray(instances) ? instances : []).forEach((instance) => {
    ...
    if (!month || !allowedTypes.has(taxType)) return;
