// FBA-CF-0027 — PO-UI mit beliebig vielen Zahlungsmeilensteinen (+Validierung)
...
function msSum100(ms){
  ...
  return Math.round(s*10)/10; // kleine Rundung
}
...
export async function render(root){
  ...
  function onAnyChange(){
    // Normalisiere Transit abhängig von Transport, wenn leer
    if (!transit.value) {
  ...
  function updateSaveEnabled(){
    const sum = msSum100(editing.milestones);
    const ok = (sum === 100) && (poNo.value.trim()!=="") && (parseDE(goods.value)>0) && !!orderDate.value;
    btnSave.disabled = !ok;
  }

  function save(){
    // schreibe editing aus Formfeldern zurück
    ...
    // validiert durch updateSaveEnabled()
    ...
    // nach Save: im Formular bleiben (bearbeiten) — oder optional zurücksetzen
  }
  ...
  transport.addEventListener("change", e=>{
    ...
    // Transit defaults bei Wechsel
    if (editing.transport==="air" && (!editing.transitDays || editing.transitDays===60 || editing.transitDays===30)) editing.transitDays = 10;
    if (editing.transport==="rail" && (!editing.transitDays || editing.transitDays===60 || editing.transitDays===10)) editing.transitDays = 30;
    if (editing.transport==="sea" && (!editing.transitDays || editing.transitDays===30 || editing.transitDays===10)) editing.transitDays = 60;
  ...
  btnNew.addEventListener("click", ()=> loadForm(defaultPO()));
  btnDel.addEventListener("click", ()=> onDelete(editing));

  // Initial render
  renderList(listZone, state.pos, onEdit, onDelete);
  loadForm(defaultPO());
}
