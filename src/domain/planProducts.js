...
function normalizeTransportMode(value) {
  ...
  if (raw === "AIR" || raw === "SEA" || raw === "RAIL") return raw;
  return "SEA";
}
...
export function normalizePlanProductRecord(raw, fallbackIndex = 0) {
  ...
  const productionLeadTimeDaysDefault = asPositiveInt(
    row.productionLeadTimeDaysDefault
  ...
  const logisticsPerUnitEur = asNumber(
    row.logisticsPerUnitEur
...
function resolveTransportTransitDays(settings, transportMode) {
  ...
  return asPositiveInt(
    leadTimes[String(transportMode || "").toLowerCase()]
    ?? leadTimes.sea,
  );
}
...
function liveImportUnitsByMonth(forecastImport, sku) {
  ...
  Object.entries(map).forEach(([monthRaw, value]) => {
    ...
    if (Number.isFinite(units)) out[month] = Number(units);
  });
  return out;
}
...
function buildMissingPlanningInputs(input) {
  const missing = [];
  if (!input.hasForecast) missing.push("forecast_units");
  if (!input.productionLeadTimeDays) missing.push("production_lead_time_days");
...
export function buildSharedPlanProductProjection(input) {
  const state = input?.state && typeof input.state === "object" ? input.state : {};
  const settings = state.settings && typeof state.settings === "object" ? state.settings : {};
  const months = Array.isArray(input?.months) && input.months.length
  ...
  rows.forEach((row, index) => {
    ...
    const plannedUnitsByMonth = isMappedBridge
      ? Object.fromEntries(
        Object.entries(rawPlannedUnitsByMonth).filter(([month]) => !(Number(liveUnitsByMonth[month]) > 0)),
      )
    ...
    const includeInForecast = normalizeIncludeInForecast(row.includeInForecast, feedActive);
    ...
    const effectiveRevenueByMonth = isMappedBridge
      ? Object.fromEntries(
        Object.entries(row.revenueByMonth || {}).filter(([month]) => bridgeMonths.has(month)),
      )
      : (row.revenueByMonth || {});
    ...
    ? {
      ...
      forecast: {
        ...
        forecastManual: {
          ...(manualBase || {}),
          ...Object.fromEntries(
            Object.entries(forecastUnitsBySkuMonth).map(([sku, byMonth]) => ([
              sku,
              {
...
export function buildPlanProductRevenueByMonthAndBucket(input) {
  ...
  projection.sharedPathEntries.forEach((row) => {
    const bucket = row.effectivePortfolioBucket || PORTFOLIO_BUCKET.PLAN;
    months.forEach((month) => {
