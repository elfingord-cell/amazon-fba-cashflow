...
export function monthEndDate(date) {
...
export function applyCnyBlackout(orderDate, prodDays, settings) {
...
export function computeGoodsTotals(record, settings) {
...
export function resolveFreightInputMode(record) {
...
export function computeFreightTotal(record, totals) {
  if (record?.timeline?.includeFreight === false) return 0;
...
export function computeAnchors(record, settings) {
...
export function resolveRates(record, settings) {
  return {
...
export function computeImportCostBreakdown({ goods, freight, rates }) {
...
export function computeVatRefundEur(eustEur, percent, vatRefundEnabled) {
  const eust = parseNumber(eustEur, 0);
  if (!vatRefundEnabled || eust <= 0) return 0;
