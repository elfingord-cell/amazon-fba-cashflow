...
function ensureAutoEvents(record, settings, manualMilestones = []) {
  normalizePoPaymentStateRecord(record, { mutate: true });
  ...
  const ensure = (type, defaults) => {
    if (!map.has(type)) {
      const created = { id: getCanonicalPoAutoEventId(record, type), type, ...defaults };
      record.autoEvents.push(created);
      map.set(type, created);
    } else {
      const current = map.get(type);
      current.id = getCanonicalPoAutoEventId(record, type);
  ...
  const firstMs = manualMilestones[0];
    ...
    enabled: coreResolveRates(record, settings).fxFeePct > 0,
  });
...
function derivePoPlanningRows(record, config, settings, options = {}) {
  ...
  const rows = [];
  const explicitAutoTypes = new Set(
    (Array.isArray(options?.explicitAutoEvents) ? options.explicitAutoEvents : [])
...
function buildPoPaymentPlanning(record, config, settings, paymentRecords = []) {
  ...
  const paymentRows = planningRows
    ...
    .map((evt) => {
      ...
      const status = hasExplicitPoPaymentEvidence({ ...log, paymentId }) ? "paid" : "open";
      return {
        id: evt.id,
        paymentInternalId,
...
export { buildPaymentRows, buildPoPaymentPlanning, getSettings };
...
function renderMsTable(container, record, config, onChange, focusInfo, settings, helpers = {}) {
  ...
  function openPaymentModal(payment) {
    ...
    function computeValidation() {
      const selectedEvents = allPayments.filter(evt => selectedIds.has(evt.id));
      return validatePaymentModalState({
        selectedEvents,
