...
function isMonthCovered(signal, allowForecast) {
  return Boolean(signal.payment || signal.inbound || (allowForecast && signal.forecast));
}
...
function computeSkuCoverage(state, months) {
  ...
  months.forEach(month => {
    ...
    activeSkus.forEach(item => {
      const sku = item.sku;
      const skuSignal = signals.has(sku) ? signals.get(sku).get(month) : null;
