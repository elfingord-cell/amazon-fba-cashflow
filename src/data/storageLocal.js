...
function ensureGlobalSettings(state) {
  ...
  const cashInRevenueBasisMode = String(
    settings.cashInRevenueBasisMode || defaults.settings.cashInRevenueBasisMode || "hybrid",
  ).trim().toLowerCase();
...
function normalizeForecastImportMap(input) {
  ...
  Object.entries(input).forEach(([skuRaw, monthMapRaw]) => {
    ...
    Object.entries(monthMapRaw).forEach(([monthRaw, entryRaw]) => {
      ...
      const revenueEur = parseDeNumber(source.revenueEur);
      const profitEur = parseDeNumber(source.profitEur);
      monthMapOut[month] = {
    ...
    if (Object.keys(monthMapOut).length) out[sku] = monthMapOut;
  });
  return out;
}
...
function ensureForecast(state) {
  ...
  if (!Array.isArray(state.forecast.versions)) state.forecast.versions = [];
  ...
  if (state.forecast.activeVersionId === undefined) state.forecast.activeVersionId = null;
  if (state.forecast.lastImpactSummary === undefined) state.forecast.lastImpactSummary = null;
