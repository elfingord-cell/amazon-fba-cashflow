...
exports.openDataHealthPanel = openDataHealthPanel;
exports.openBlockingModal = openBlockingModal;
exports.initDataHealthUI = initDataHealthUI;
...
function computeHealth() {
    ...
    return (0, dataHealth_js_1.validateAll)({
...
function openDataHealthPanel(filter = {}) {
    const { issues } = computeHealth();
    const filtered = issues.filter(issue => {
        if (filter.scope && issue.scope !== filter.scope)
            return false;
        if (filter.entityId && issue.entityId !== filter.entityId)
            return false;
        return true;
    });
...
function openBlockingModal(issues) {
