...
exports.getSuggestedInvoiceFilename = getSuggestedInvoiceFilename;
...
function normalizePaymentType(typeLabel, eventType) {
    ...
    if (label.includes("eust"))
        return "EUSt";
    if (label.includes("zoll") || label.includes("duty"))
        return "Zoll";
    if (label.includes("other") || label.includes("sonst"))
        return "Other";
    return "Other";
}
function getSuggestedInvoiceFilename(po, payment, options = {}) {
    if (!po || !payment)
        return "";
