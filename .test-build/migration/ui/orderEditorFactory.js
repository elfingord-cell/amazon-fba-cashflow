...
const dataTable_js_1 = require("./components/dataTable.js");
const dataHealth_js_1 = require("../lib/dataHealth.js");
const dataHealthUi_js_1 = require("./dataHealthUi.js");
const numberFormat_js_1 = require("./utils/numberFormat.js");
const paymentValidation_js_1 = require("./utils/paymentValidation.js");
const dateUtils_js_1 = require("../lib/dateUtils.js");
const invoiceFilename_js_1 = require("./utils/invoiceFilename.js");
const freightEstimate_js_1 = require("../domain/costing/freightEstimate.js");
...
const useDraftForm_js_1 = require("../hooks/useDraftForm.js");
const useDirtyGuard_js_1 = require("../hooks/useDirtyGuard.js");
...
function refreshProductCache() {
    productCache = (0, storageLocal_js_1.getProductsSnapshot)();
}
...
function ensureTimeline(record, settings = getSettings()) {
    if (!record)
        return {};
    if (!record.timeline || typeof record.timeline !== "object")
        record.timeline = {};
    if (record.timeline.includeFreight == null)
        record.timeline.includeFreight = true;
...
function updateDerivedFreight(record, settings = getSettings()) {
    ...
    const estimate = (0, freightEstimate_js_1.computeFreightEstimate)({
        ...record,
        timeline,
    }, getProductsBySku());
    ...
    estimate.lines.forEach(line => {
        ...
        if (line.sku)
            lineMap.set(`sku:${String(line.sku).toLowerCase()}`, line);
    });
...
function getMergedPayments(record) {
    const stateSnapshot = (0, store_js_1.loadAppState)();
...
function prepareRecordForDraft(record, settings) {
    const next = (0, safeDeepClone_js_1.safeDeepClone)(record || {});
...
function formatAutoFreightTooltip(line) {
    if (!line)
        return [];
    const alias = productCache.find(prod => prod?.sku?.trim().toLowerCase() === String(line.sku || "").trim().toLowerCase())?.alias;
    const rows = [
        `SKU: ${line.sku || "—"}`,
        `Alias: ${alias || "—"}`,
...
function formatFreightMissingFields(fields) {
    ...
    const labels = fields.map((field) => {
        if (field === "landedCostEur")
            return "Einstandspreis";
        if (field === "unitPriceUsd")
            return "Unit Price";
        if (field === "fxUsdPerEur")
            return "FX";
        return field;
    });
...
function formatSkuSummary(record) {
    ...
    if (!items.length)
        return productLabelForList(record?.sku);
...
function parseDE(value) {
    const parsed = (0, numberFormat_js_1.parseLocalizedNumber)(value);
...
function fmtEURPlain(value) {
    ...
    return Number.isFinite(num) ? (0, numberFormat_js_1.formatMoneyDE)(num, 2) : "0,00";
}
function fmtCurrencyInput(value) {
    ...
    if (!Number.isFinite(parsed))
        return raw;
    return (0, numberFormat_js_1.formatLocalizedNumber)(parsed, 2, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
...
function getCnyWindow(settings, year) {
    ...
    if (direct?.start && direct?.end) {
        const start = (0, dateUtils_js_1.parseISODate)(direct.start);
        const end = (0, dateUtils_js_1.parseISODate)(direct.end);
...
function applyCnyBlackout(orderDate, prodDays, settings) {
    ...
    const prodEnd = (0, dateUtils_js_1.addDays)(orderDate, baseDays);
    if (!prodEnd)
        return { prodDone: orderDate, adjustmentDays: 0, cnyStart: null, cnyEnd: null };
    ...
    for (let year = startYear; year <= endYear; year += 1) {
        ...
        if (!window)
            continue;
        const overlap = (0, dateUtils_js_1.overlapDays)(orderDate, prodEnd, window.start, window.end);
        if (overlap > 0) {
    ...
    const adjustedEnd = adjustmentDays ? (0, dateUtils_js_1.addDays)(prodEnd, adjustmentDays) : prodEnd;
    return {
    ...
    for (const field of [...QUICKFILL_FIELD_MAP, ...QUICKFILL_ARRAY_FIELDS]) {
        ...
        const format = (value) => {
            ...
            if (currencyFields.has(field)) {
                if (field === "freightEur" || field === "freightPerUnitEur")
                    return fmtEUR(parseDE(value));
                return fmtUSD(parseDE(value));
            }
            if (percentFields.has(field))
                return `${fmtPercent(value)} %`;
            if (field === "fxOverride")
                return fmtFxRate(value);
            if (field === "freightMode")
                return value === "per_unit" ? "Pro Stück" : "Gesamt";
...
function computeEstimatedFreightTotals(record, settings = getSettings()) {
    if (!record)
        return { total: 0, missing: 0, used: [], estimate: null };
        ...
        for (const evt of record.autoEvents) {
            if (evt.type === "freight" || evt.type === "duty" || evt.type === "eust" || evt.type === "vat_refund") {
                if (evt.enabled !== false)
                    evt._ddpEnabledBackup = evt.enabled !== false;
                evt.enabled = false;
            }
        }
    }
...
function ensurePaymentLog(record) {
    if (!record)
        return {};
    if (!record.paymentLog || typeof record.paymentLog !== "object")
        record.paymentLog = {};
...
function ensurePaymentInternalId(record, eventId) {
    ...
    if (!log[eventId] || typeof log[eventId] !== "object")
        log[eventId] = {};
...
function mapPaymentType(evt, milestone) {
    ...
    if (label.includes("deposit") || label.includes("anzahlung"))
        return "Deposit";
    if (label.includes("balance") || label.includes("rest"))
        return "Balance";
    if (label.includes("shipping") || label.includes("fracht"))
        return "Fracht";
    return "Other";
}
function buildInvoiceKeyEvents(selectedEvents) {
    ...
    if (hasDeposit && hasBalance)
        return `Deposit+Balance${hasFx ? "+FX" : ""}`;
    if (hasDeposit && hasFx)
        return "Deposit+FX";
    if (hasDeposit)
        return "Deposit";
    if (hasBalance)
        return hasFx ? "Balance+FX" : "Balance";
    if (hasShipping)
        return "Fracht";
    if (hasEust)
        return "EUSt";
    if (hasFx && lowered.length === 1)
        return "FX";
    if (labels.length <= 1)
        return labels[0] || "Payment";
...
function anchorDate(schedule, anchor) {
    if (!schedule)
        return null;
    if (anchor === "ORDER_DATE")
        return schedule.order;
    if (anchor === "PROD_DONE")
        return schedule.prodDone;
    if (anchor === "ETD")
        return schedule.etd;
    return schedule.eta;
}
...
function classifyManualMilestoneType(label) {
    ...
    if (text.includes("deposit") || text.includes("anzahlung"))
        return "deposit";
    if (text.includes("balance") || text.includes("rest"))
        return "balance";
    return "milestone";
}
...
function buildEventList(events, paymentLog = {}, paymentRecords = []) {
    ...
    const paymentMap = buildPaymentMap(paymentRecords);
    wrapper.append(el("div", { class: "po-event-head" }, [
    ...
    for (const evt of events) {
        ...
        wrapper.append(el("div", { class: "po-event-row" }, [
...
function renderList(container, records, config, onEdit, onDelete, options = {}) {
    ...
    const table = (0, dataTable_js_1.createDataTable)({
        columns,
        rows: listRows,
...
function renderPoList(container, records, config, onEdit, onDelete, options = {}) {
    ...
    function updateArchive(rec, archived) {
        const st = (0, store_js_1.loadAppState)();
        ...
        if (idx >= 0) {
            ...
            (0, store_js_1.commitAppState)(st, { source: `${config.slug}:archive`, entityKey: `${config.slug}:${rec[config.numberField] || rec.id}`, action: "update" });
    ...
    return [
        {
            ...
            defaultChecked: true,
            formatter: (v) => v != null ? `${(0, numberFormat_js_1.formatLocalizedNumber)(v, 2, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} $` : "—",
        },
        {
            key: "extraPerUnitUsd",
        ...
        },
        {
            ...
            defaultChecked: false,
            formatter: (v) => v != null ? `${(0, numberFormat_js_1.formatLocalizedNumber)(v, 2, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} %` : "—",
        },
        {
            key: "vatImportPct",
            label: "EUSt (%)",
        ...
        },
        {
            ...
            defaultChecked: false,
            formatter: (v) => v != null ? (0, numberFormat_js_1.formatLocalizedNumber)(v, 4, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) : "—",
        },
        {
            key: "fxFeePct",
            label: "FX-Gebühr (%)",
...
function openApplyToMasterDialog({ item, record, line, settings, onApplied, showToast }) {
    ...
    if (!sku) {
        if (typeof showToast === "function")
            showToast("Bitte zuerst eine SKU für diese Position eintragen.");
        return;
    }
    const product = (0, storageLocal_js_1.getProductBySku)(sku);
    if (!product) {
        if (typeof showToast === "function")
            showToast(`SKU "${sku}" nicht in Stammdaten gefunden.`);
        return;
    }
    ...
    const buildRow = (cand) => {
        ...
        const isUnchanged = oldValue != null && cand.value != null && (typeof cand.value === "boolean"
            ? oldValue === cand.value
            ...
            el("button", {
                ...
                onclick: () => {
                    ...
                    if (!selectedKeys.size) {
                        if (typeof showToast === "function")
                            showToast("Bitte mindestens ein Feld auswählen.");
                        return;
                    }
                    try {
                        const nextProduct = applyCandidatesToProduct(product, candidates, selectedKeys);
                        (0, storageLocal_js_1.upsertProduct)(nextProduct, { source: "po-master-update" });
                        refreshProductCache();
                        close();
                        if (typeof showToast === "function") {
...
function renderItemsTable(container, record, onChange, dataListId, helpers = {}) {
    ...
    const applyAutoUnitCost = (item, input, { force } = {}) => {
        if (!item || !input)
            return;
        if (item.unitCostManuallyEdited)
            return;
    ...
    record.items.forEach(item => {
        ...
        applyToMasterBtn.addEventListener("click", () => {
            ...
            openApplyToMasterDialog({
                ...
                onApplied: () => {
                    if (typeof helpers.onMasterUpdated === "function")
                        helpers.onMasterUpdated();
                },
            });
        });
        ...
        row.append(el("td", {}, [skuInput]), el("td", {}, [unitsInput]), el("td", {}, [unitCostInput]), el("td", {}, [unitExtraInput]), el("td", {}, [flatInput]), el("td", { dataset: { logisticsCell: "true" } }, [logisticsCell]), el("td", { class: "po-items-actions-cell" }, [applyToMasterBtn, removeBtn]));
        body.append(row);
    });
    const table = el("table", {
...
function computeOrderDateForEta(anchorDate, record, settings = getSettings()) {
    ...
    let candidate = (0, dateUtils_js_1.addDays)(anchorDate, -baseTotal);
    const blackout = applyCnyBlackout(candidate, prodDays, settings);
    if (blackout.adjustmentDays) {
        candidate = (0, dateUtils_js_1.addDays)(candidate, -blackout.adjustmentDays);
    }
    return candidate;
}
...
function renderMsTable(container, record, config, onChange, focusInfo, settings, helpers = {}) {
                ...
                el("label", { class: "inline-checkbox" }, [
                    el("input", {
                        ...
                        onchange: (e) => {
                            ...
                            if (!checked)
                                autoEvt._ddpEnabledBackup = false;
                            else
                                delete autoEvt._ddpEnabledBackup;
                            onChange();
                        },
                    }),
                    " Aktiv",
    ...
    function openPaymentModal(payment) {
        ...
        if (Number.isFinite(Number(paymentRecord?.amountActualEurTotal))) {
            actualInput.value = (0, numberFormat_js_1.formatMoneyDE)(Number(paymentRecord.amountActualEurTotal), 2);
        }
        else if (Number.isFinite(Number(payment?.paidEurActual))) {
            actualInput.value = (0, numberFormat_js_1.formatMoneyDE)(Number(payment.paidEurActual), 2);
        }
        ...
        function renderAllocation() {
            ...
            const total = (0, numberFormat_js_1.parseMoneyInput)(actualInput.value);
            const allocations = Number.isFinite(total) ? (0, paymentValidation_js_1.allocatePayment)(total, selectedEvents) : null;
            selectedEvents.forEach(evt => {
                const allocation = allocations?.find(entry => entry.eventId === evt.id);
                allocationBody.append(el("tr", {}, [
                    el("td", {}, [evt.label]),
                    el("td", {}, [fmtEURPlain(evt.plannedEur)]),
                    el("td", {}, [allocation ? (0, numberFormat_js_1.formatMoneyDE)(allocation.actual, 2) : "—"]),
                ]));
            });
        }
        ...
        if (currentPaymentId) {
            Object.entries(record.paymentLog || {}).forEach(([eventId, log]) => {
                if (log?.paymentId === currentPaymentId)
                    selectedIds.add(eventId);
            });
        }
        if (!selectedIds.size && payment?.id)
            selectedIds.add(payment.id);
        const toggleSelection = (evtId, force) => {
            if (force === true)
                selectedIds.add(evtId);
            else if (force === false)
                selectedIds.delete(evtId);
            else if (selectedIds.has(evtId))
                selectedIds.delete(evtId);
            else
                selectedIds.add(evtId);
            updateSummary();
        ...
        const updateLinkButtons = () => {
            const invoiceUrl = (0, paymentValidation_js_1.normalizeUrl)(invoiceUrlInput.value);
            const folderUrl = (0, paymentValidation_js_1.normalizeUrl)(folderUrlInput.value);
            const invoiceValid = invoiceUrl && (0, paymentValidation_js_1.isHttpUrl)(invoiceUrl);
            const folderValid = folderUrl && (0, paymentValidation_js_1.isHttpUrl)(folderUrl);
        ...
        const eventsError = el("div", { class: "form-error po-payment-field-error", dataset: { field: "events" } }, []);
        const actualError = el("div", { class: "form-error po-payment-field-error", dataset: { field: "actual" } }, []);
        const paymentIdError = el("div", { class: "form-error po-payment-field-error", dataset: { field: "paymentId" } }, []);
        const invoiceError = el("div", { class: "form-error po-payment-field-error", dataset: { field: "invoiceUrl" } }, []);
        const folderError = el("div", { class: "form-error po-payment-field-error", dataset: { field: "folderUrl" } }, []);
        function getModalState() {
            return {
                ...
                invoiceUrl: (0, paymentValidation_js_1.normalizeUrl)(invoiceUrlInput.value) || "",
                folderUrl: (0, paymentValidation_js_1.normalizeUrl)(folderUrlInput.value) || "",
        ...
        function computeValidation() {
            const selectedEvents = allPayments.filter(evt => selectedIds.has(evt.id));
            return (0, paymentValidation_js_1.validatePaymentModalState)({
                selectedEvents,
                actualRaw: actualInput.value.trim(),
                invoiceUrl: (0, paymentValidation_js_1.normalizeUrl)(invoiceUrlInput.value),
                folderUrl: (0, paymentValidation_js_1.normalizeUrl)(folderUrlInput.value),
                paymentRecord,
                paymentIdValue: paymentIdInput.value,
                mergedPayments,
            });
        }
        ...
        function applyFieldErrors(fieldErrors = {}) {
            eventsError.textContent = fieldErrors.events || "";
            actualError.textContent = fieldErrors.actual || "";
            paymentIdError.textContent = fieldErrors.paymentId || "";
            invoiceError.textContent = fieldErrors.invoiceUrl || "";
            folderError.textContent = fieldErrors.folderUrl || "";
            eventList.classList.toggle("input-error", Boolean(fieldErrors.events));
            actualInput.classList.toggle("input-error", Boolean(fieldErrors.actual));
            paymentIdInput.classList.toggle("input-error", Boolean(fieldErrors.paymentId));
            invoiceUrlInput.classList.toggle("input-error", Boolean(fieldErrors.invoiceUrl));
            folderUrlInput.classList.toggle("input-error", Boolean(fieldErrors.folderUrl));
        }
        function updateSaveState() {
            const validation = computeValidation();
            const isDirty = !(0, deepEqual_js_1.deepEqual)(getModalState(), initialState);
            ...
            if (!validation.valid) {
                ...
                setFormError(validation.reason || "Bitte Pflichtfelder ausfüllen");
            }
        ...
        saveBtn.addEventListener("click", () => {
            ...
            if (!validation.valid) {
                console.warn("[po-payment-modal] validation failed", {
                    reason: validation.reason || null,
                    selectedCount: validation.selectedEvents?.length || 0,
                    sumPlanned: validation.sumPlanned || 0,
                });
            ...
            try {
                ...
                console.info("[po-payment-modal] payment marked as paid", {
                    ...
                    sumPlanned: validation.sumPlanned,
    ...
    payments.forEach(payment => {
        ...
        const delta = payment.paidEurActual != null
            ...
            : null;
        const suggestedFilename = (0, invoiceFilename_js_1.getSuggestedInvoiceFilename)(record, {
        ...
        const row = el("tr", { dataset: { paymentId: payment.id, paymentType: payment.typeLabel, paymentEventType: payment.eventType || "" } }, [
            ...
            el("td", {}, [
                el("div", { style: "display:flex;gap:6px;flex-wrap:wrap;" }, [
                    ...
                    (0, paymentValidation_js_1.isHttpUrl)((0, paymentValidation_js_1.normalizeUrl)(payment.invoiceDriveUrl))
                        ? el("a", {
                            class: "btn secondary sm",
                            href: (0, paymentValidation_js_1.normalizeUrl)(payment.invoiceDriveUrl),
                            target: "_blank",
                            rel: "noopener noreferrer",
                        }, ["Open Invoice"])
                        : null,
                    (0, paymentValidation_js_1.isHttpUrl)((0, paymentValidation_js_1.normalizeUrl)(payment.invoiceFolderDriveUrl))
                        ? el("a", {
                            class: "btn secondary sm",
                            href: (0, paymentValidation_js_1.normalizeUrl)(payment.invoiceFolderDriveUrl),
                            target: "_blank",
                            rel: "noopener noreferrer",
                        }, ["Open Folder"])
                        : null,
...
function renderOrderModule(root, config) {
    const state = (0, store_js_1.loadAppState)();
    if (!Array.isArray(state[config.entityKey]))
        state[config.entityKey] = [];
  ...
  `;
    ...
    let draftForm = (0, useDraftForm_js_1.useDraftForm)(prepareRecordForDraft(defaultRecord(config, getSettings()), getSettings()), {
        key: `${config.slug}:new`,
    ...
    let isDirty = false;
    const dirtyGuard = (0, useDirtyGuard_js_1.useDirtyGuard)(() => draftForm.isDirty, "Ungespeicherte Änderungen verwerfen?");
    dirtyGuard.register();
    ...
    function resolveProductFromInput(value) {
        ...
        return productCache.find(prod => {
            ...
            if (term === combo.toLowerCase() || baseTerm === combo.toLowerCase())
                return true;
            if (supplier && term === supplier)
                return true;
            if (alias && (term === alias || baseTerm === alias))
                return true;
            if (sku && (term === sku || baseTerm === sku))
                return true;
            return false;
        }) || null;
    }
    ...
    function renderRecentChips() {
        ...
        if (!container)
            return;
        const recents = (0, storageLocal_js_1.getRecentProducts)();
        container.innerHTML = "";
        if (!recents.length) {
    ...
    function hasUnsavedChanges() {
        ...
        const dirty = !(0, deepEqual_js_1.deepEqual)(snapshot, lastLoaded);
        if (dirty) {
            editing = snapshot;
    ...
    function confirmDiscard(onConfirm) {
        (0, confirmDialog_js_1.openConfirmDialog)({
    ...
    function getCurrentState() {
        return (0, store_js_1.loadAppState)();
    }
    function getAllRecords() {
        const st = getCurrentState();
        if (!Array.isArray(st[config.entityKey]))
            st[config.entityKey] = [];
    ...
    function renderListView(recordsOverride, update = null) {
        ...
        if (poMode && update) {
            if (Object.prototype.hasOwnProperty.call(update, "sortKey"))
                poListState.sortKey = update.sortKey;
            if (Object.prototype.hasOwnProperty.call(update, "sortDir"))
                poListState.sortDir = update.sortDir;
        }
    ...
    function getHistoryFor(skuValue, supplierValue, limit = 10) {
        ...
        const matches = getAllRecords().filter(rec => {
            ...
            if (!recSku && !itemMatch)
                return false;
            if (recSku && recSku !== skuLower && !itemMatch)
                return false;
            if (!supplierLower)
                return true;
    ...
    function refreshSkuOptions() {
        ...
        productCache
            ...
            if (showBlockedProducts)
                return true;
            const completeness = (0, productCompleteness_js_1.evaluateProductCompleteness)(prod, { state: getCurrentState() });
    ...
    function applySourceRecord(source, message) {
        ...
        if (parsedSku) {
            ...
            if (product) {
                setSkuField(product);
                (0, storageLocal_js_1.recordRecentProduct)(product.sku);
            }
        }
        if (message)
    ...
    function renderDiffList(diffs, container) {
        ...
        diffs.forEach(diff => {
            list.append(el("dt", {}, [diff.label]), el("dd", {}, [diff.type === "list"
    ...
    function openHistoryModal() {
        ...
        if (footer) {
            footer.innerHTML = "";
            footer.append(el("button", { class: "btn", type: "button", onclick: () => closeModal(overlay) }, ["Abbrechen"]));
        }
        const settings = getSettings();
        ...
        records.slice(0, 10).forEach((rec, index) => {
            ...
            tableBody.append(el("tr", { class: index === 0 ? "po-history-latest" : "" }, [
    ...
    function openTemplateModal() {
        ...
        if (footer) {
            ...
            saveBtn.addEventListener("click", () => {
                ...
                (0, storageLocal_js_1.upsertProduct)(payload);
                (0, storageLocal_js_1.recordRecentProduct)(skuValue);
                refreshProductCache();
                refreshQuickfillControls();
    ...
    function openCreateProductModal() {
        ...
        form.append(el("label", {}, ["SKU", skuField]), el("label", {}, ["Alias", aliasField]), el("label", {}, ["Supplier", supplierField]));
        ...
        if (footer) {
            ...
            saveBtn.addEventListener("click", () => {
                ...
                try {
                    (0, storageLocal_js_1.upsertProduct)({
                        sku,
                        alias,
                    ...
                    (0, storageLocal_js_1.recordRecentProduct)(sku);
                    refreshProductCache();
                    if (skuInput) {
    ...
    function updateTimelineOverrides(timeline) {
        if (etdComputed)
            etdComputed.textContent = fmtDateDE(timeline?.etdComputed);
        if (etaComputed)
            etaComputed.textContent = fmtDateDE(timeline?.etaComputed);
        if (etdResetBtn)
            etdResetBtn.disabled = !editing.etdManual;
        if (etaResetBtn)
            etaResetBtn.disabled = !editing.etaManual;
    }
    ...
    function updateFreightModeUI() {
        if (!freightModeSelect || !freightInput || !freightPerUnitInput)
            return;
    ...
    function resolveFreightPerUnitSuggestion(product, settings) {
        ...
        return (0, costing_js_1.computeFreightPerUnitEur)({
            unitPriceUsd,
            landedCostEur,
            fxUsdPerEur,
        });
    ...
    function updateFreightPerUnitSuggestion(product, { force = false } = {}) {
        ...
        if (!product) {
            if (freightPerUnitSuggested)
                freightPerUnitSuggested.textContent = "";
            if (freightPerUnitWarning)
                freightPerUnitWarning.textContent = "";
            return;
        }
        const settings = getSettings();
    ...
    function applyProductDefaultsFromProduct(product) {
        ...
        const prefill = (0, prefill_js_1.buildPrefillForSku)(product.sku, {
            mode: "PO",
        ...
        const setNumberIfEmpty = (key, value) => {
            if (value == null || !isEmptyNumber(next[key]))
                return;
            next[key] = value;
        };
        ...
        if (Array.isArray(next.items)) {
            next.items = next.items.map(item => {
                if (!item || (item.sku && item.sku !== product.sku))
                    return item;
                const updated = { ...item };
        ...
        const isDefaultMilestones = (milestones) => {
            if (!Array.isArray(milestones) || milestones.length !== defaultMilestones.length)
                return false;
    ...
    function updateOrderDateFields(iso) {
        ...
        if (orderDateInput)
            orderDateInput.value = safeIso;
        if (orderDateDisplay)
            orderDateDisplay.value = fmtDateDE(safeIso);
        if (orderDateError)
    ...
    function validateOrderDate(showError = false) {
        ...
        if (orderDateInput)
            orderDateInput.value = isoVal;
        if (orderDateDisplay)
            orderDateDisplay.value = fmtDateDE(isoVal);
        return isoVal;
    }
    function applyDraftToForm() {
        ...
        if (!editing.paymentDrafts)
            editing.paymentDrafts = {};
        if (poStatus) {
        ...
        if (quickfillEnabled) {
            ...
            if (supplierInput)
                supplierInput.value = editing.supplier || "";
            setQuickStatus("");
            refreshQuickfillControls();
        }
        ...
        if (fxRateInput)
            fxRateInput.value = fxBase ? fmtFxRate(fxBase) : "";
        ...
        if (freightModeSelect)
            freightModeSelect.value = resolveFreightInputMode(editing);
        freightInput.value = fmtCurrencyInput(editing.freightEur ?? "0,00");
        if (freightPerUnitInput)
            freightPerUnitInput.value = fmtCurrencyInput(editing.freightPerUnitEur ?? "0,00");
        ...
        if (etdManualInput)
            etdManualInput.value = editing.etdManual || "";
        if (etaManualInput)
            etaManualInput.value = editing.etaManual || "";
    ...
    function loadForm(record) {
        setPrefillBanner([]);
        const settings = getSettings();
        draftForm = (0, useDraftForm_js_1.useDraftForm)(prepareRecordForDraft(record, settings), { key: entityKeyFor(record), enableDraftCache: true });
        editing = draftForm.draft;
        lastLoaded = (0, safeDeepClone_js_1.safeDeepClone)(editing);
        applyDraftToForm();
        draftForm.markClean();
    ...
    async function saveRecord() {
        ...
        normaliseArchiveFlag(editing);
        const stateSnapshot = (0, store_js_1.loadAppState)();
        const { issues } = (0, dataHealth_js_1.validateAll)({
        ...
        if (!editing.supplier) {
            blocking.push((0, dataHealth_js_1.makeIssue)({
                scope: "po",
        ...
        if (blocking.length) {
            (0, dataHealthUi_js_1.openBlockingModal)(blocking);
            return;
        }
        await draftForm.commit((draft) => {
            const st = (0, store_js_1.loadAppState)();
            const nextRecord = (0, safeDeepClone_js_1.safeDeepClone)(draft);
            ...
            const nextPayments = basePayments.map(payment => (0, safeDeepClone_js_1.safeDeepClone)(payment));
            Object.values(paymentDrafts).forEach(payload => {
                if (!payload?.id)
                    return;
                const idx = nextPayments.findIndex(entry => entry?.id === payload.id);
                const nextPayload = (0, safeDeepClone_js_1.safeDeepClone)(payload);
                if (idx >= 0)
                    nextPayments[idx] = { ...nextPayments[idx], ...nextPayload };
                else
                    nextPayments.push(nextPayload);
            });
            ...
            if (idx >= 0)
                ...
                arr.push(nextRecord);
            st[config.entityKey] = arr;
            (0, store_js_1.commitAppState)(st, {
                source: `${config.slug}:save`,
                entityKey: entityKeyFor(nextRecord),
                action: idx >= 0 ? "update" : "create",
            });
            lastLoaded = (0, safeDeepClone_js_1.safeDeepClone)(nextRecord);
        });
        renderListView((0, store_js_1.loadAppState)()[config.entityKey]);
        refreshQuickfillControls();
        if (quickfillEnabled && editing.sku) {
            (0, storageLocal_js_1.recordRecentProduct)(editing.sku);
        }
    ...
    async function convertRecord() {
        ...
        await saveRecord();
        const st = (0, store_js_1.loadAppState)();
        if (!Array.isArray(st[config.convertTo.entityKey]))
            st[config.convertTo.entityKey] = [];
    ...
    if (etdManualInput)
        etdManualInput.addEventListener("change", onAnyChange);
    if (etaManualInput)
        etaManualInput.addEventListener("change", onAnyChange);
    if (etdResetBtn) {
        etdResetBtn.addEventListener("click", () => {
            if (etdManualInput)
                etdManualInput.value = "";
    ...
    transportSelect.addEventListener("change", (e) => {
        ...
        if (editing.transport === "air")
            editing.transitDays = 10;
        if (editing.transport === "rail")
            editing.transitDays = 30;
        if (editing.transport === "sea")
            editing.transitDays = 60;
    ...
    if (quickfillEnabled && skuInput) {
        const handleSkuChange = () => {
            ...
            if (product) {
                ...
                editing.sku = product.sku;
                (0, storageLocal_js_1.recordRecentProduct)(product.sku);
                const completeness = (0, productCompleteness_js_1.evaluateProductCompleteness)(product, { state: getCurrentState() });
    ...
    if (orderDateDisplay)
        orderDateDisplay.addEventListener("input", onAnyChange);
    ...
    if (convertBtn)
        convertBtn.addEventListener("click", () => convertRecord());
    ...
    const handleStateChanged = () => {
        const freshState = (0, store_js_1.loadAppState)();
        const settings = getSettings();
