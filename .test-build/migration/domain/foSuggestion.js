...
exports.foSuggestionUtils = void 0;
exports.getLatestClosingSnapshotMonth = getLatestClosingSnapshotMonth;
exports.buildSkuProjection = buildSkuProjection;
exports.computeFoRecommendation = computeFoRecommendation;
exports.computeFoSuggestion = computeFoSuggestion;
exports.findEtaForMinDoh = findEtaForMinDoh;
...
function resolveSkuPolicy(sku, overrides = {}, defaults = {}) {
    ...
    return {
        ...
        operationalCoverageDaysDefault: override.operationalCoverageDaysOverride ?? defaults.operationalCoverageDaysDefault ?? 120,
    };
}
...
function countOverlapDays(windowStart, windowEnd, blackoutStart, blackoutEnd) {
    if (!windowStart || !windowEnd || !blackoutStart || !blackoutEnd)
        return 0;
...
function getLatestClosingSnapshotMonth(snapshots = []) {
    let latest = null;
...
function buildSkuProjection({ sku, baselineMonth, stock0 = 0, forecastByMonth = {}, inboundByMonth = {}, horizonMonths = 12, }) {
    const months = [];
...
function computeFoRecommendation({ sku, baselineMonth, projection, plannedSalesBySku = {}, safetyStockDays = 60, coverageDays = 90, leadTimeDays = 0, cnyPeriod, inboundWithoutEtaCount = 0, moqUnits = 0, unitsPerCarton = null, roundupCartonBlock = null, roundupMaxPct = null, requiredArrivalMonth = null, }) {
    if (!baselineMonth) {
        return {
            sku,
            status: "no_snapshot",
    ...
    const firstRisk = projection?.months?.find(entry => Number.isFinite(entry.doh) && entry.doh < safetyStockDays);
    if (!firstRisk) {
        return {
            sku,
            status: "no_fo_needed",
    ...
    const projectionByMonth = new Map((projection?.months || []).map((entry) => [entry.month, entry]));
    const selectedArrivalMonth = (requiredArrivalMonth
...
function computeFoSuggestion({ sku, today = new Date(), operationalCoverageDays, etaDate, policyOverrides, policyDefaults, plannedSalesBySku, closingStockBySku, }) {
    ...
    const orderNeededFlag = dohEom != null &&
...
function findEtaForMinDoh({ sku, today = new Date(), minimumDohDays, plannedSalesBySku, closingStockBySku, maxHorizonDays = 365, }) {
    const warnings = [];
