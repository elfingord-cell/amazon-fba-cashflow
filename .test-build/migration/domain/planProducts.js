...
exports.monthNumberToLabel = monthNumberToLabel;
...
function normalizePlanProductRecord(raw, fallbackIndex = 0) {
    ...
    const includeInForecast = (0, portfolioBuckets_js_1.normalizeIncludeInForecast)(row.includeInForecast, normalizedStatus === "active");
    return {
        ...
        portfolioBucket: (0, portfolioBuckets_js_1.normalizePortfolioBucket)(row.portfolioBucket, portfolioBuckets_js_1.PORTFOLIO_BUCKET.PLAN),
        includeInForecast,
        launchCosts: (0, portfolioBuckets_js_1.normalizeLaunchCosts)(row.launchCosts, `plan-lc-${fallbackIndex + 1}`),
...
function normalizePlanProductMappingRecord(raw, fallbackIndex = 0) {
    const row = raw && typeof raw === "object" ? raw : {};
...
function resolveForecastSkuMap(forecastImport, sku) {
    if (!sku || !forecastImport || typeof forecastImport !== "object")
        return null;
...
function computeSeasonalityFromForecastImport(forecastImport, sku) {
    ...
    const points = Object.entries(monthMap)
        ...
        const units = payload && typeof payload === "object"
            ? (0, dataHealth_js_1.parseDeNumber)(payload.units)
            : (0, dataHealth_js_1.parseDeNumber)(payload);
    ...
    for (let monthNumber = 1; monthNumber <= 12; monthNumber += 1) {
        ...
        factorsByMonthNumber[monthNumber] = (Number.isFinite(average)
...
function projectPlanBaseUnitsForMonth(input) {
    ...
    if (baseline == null || baseline < 0 || refMonth == null || targetMonth == null)
        return null;
    const factorRef = asNumber(factors[refMonth]);
    const factorTarget = asNumber(factors[targetMonth]);
    if (!Number.isFinite(factorRef) || !Number.isFinite(factorTarget) || factorRef <= 0)
        return null;
...
function rampFactorForDate(input) {
    ...
    if (date < launchDate)
        return 0;
    if (!(rampEndDate instanceof Date) || Number.isNaN(rampEndDate.getTime()))
        return 1;
    if (date > rampEndDate)
        return 1;
...
function buildPlanProductForecastRow(input) {
    ...
    months.forEach((month) => {
        ...
        revenueByMonth[month] = (Number.isFinite(price)
...
function buildPlanProductForecastRows(input) {
...
function buildPlanProductRevenueByMonthAndBucket(input) {
    ...
    const totalsByMonth = {};
    const byBucket = portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.reduce((acc, bucket) => {
        acc[bucket] = {};
        return acc;
    }, {});
    months.forEach((month) => {
        totalsByMonth[month] = 0;
        portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.forEach((bucket) => {
...
function buildPlanVsLiveComparisonRows(input) {
    ...
    return filteredMonths
        ...
        const deltaUnits = (Number.isFinite(planUnits)
            && Number.isFinite(liveUnits)) ? Number(liveUnits) - Number(planUnits) : null;
        const deltaPct = (Number.isFinite(deltaUnits)
            && Number.isFinite(planUnits)
            && Number(planUnits) > 0) ? (Number(deltaUnits) / Number(planUnits)) * 100 : null;
        return {
            month,
...
function monthNumberToLabel(monthNumber) {
