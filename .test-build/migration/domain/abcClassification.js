...
exports.computeAbcClassification = computeAbcClassification;
const dataHealth_js_1 = require("../lib/dataHealth.js");
const monthRange_js_1 = require("../utils/monthRange.js");
...
function getForecastUnits(state, sku, month) {
    ...
    const manualParsed = (0, dataHealth_js_1.parseDeNumber)(manualValue);
    if (Number.isFinite(manualParsed))
        return manualParsed;
    const importedValue = state?.forecast?.forecastImport?.[sku]?.[month]?.units;
    const importParsed = (0, dataHealth_js_1.parseDeNumber)(importedValue);
    if (Number.isFinite(importParsed))
        return importParsed;
    return null;
}
...
function computeAbcClassification(state) {
    const products = Array.isArray(state?.products) ? state.products : [];
    const allMonths = collectForecastMonths(state);
    const months = (0, monthRange_js_1.getVisibleMonths)(allMonths, "NEXT_6", (0, monthRange_js_1.currentMonthKey)(), monthRange_js_1.DASHBOARD_RANGE_OPTIONS);
    const bySku = new Map();
    ...
    products.forEach(product => {
        ...
        const price = (0, dataHealth_js_1.parseDeNumber)(product?.avgSellingPriceGrossEUR);
        let units6m = 0;
