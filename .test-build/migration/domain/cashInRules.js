...
exports.REVENUE_CALIBRATION_LIVE_HORIZON_MONTHS = exports.REVENUE_CALIBRATION_LIVE_FACTOR_MAX = exports.REVENUE_CALIBRATION_LIVE_FACTOR_MIN = exports.REVENUE_CALIBRATION_DEFAULT_RISK = exports.REVENUE_CALIBRATION_DEFAULT_BIAS = exports.REVENUE_CALIBRATION_RISK_MAX = exports.REVENUE_CALIBRATION_CONSERVATIVE_MAX = exports.REVENUE_CALIBRATION_CONSERVATIVE_MIN = exports.REVENUE_CALIBRATION_BIAS_MAX = exports.REVENUE_CALIBRATION_BIAS_MIN = exports.REVENUE_CALIBRATION_GAMMA = exports.REVENUE_CALIBRATION_ALPHA = exports.CASH_IN_SEASONALITY_PRIOR_DEFAULT_BY_MONTH = exports.CASH_IN_SEASONALITY_PROFILE_SOURCE_LABEL = exports.CASH_IN_PLAN_SAFETY_MARGIN_PCT = exports.CASH_IN_SHRINKAGE_MIN_SAMPLES = exports.CASH_IN_RISK_CAP_PCT = exports.CASH_IN_SEASONALITY_CAP_PCT = exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT = exports.CASH_IN_QUOTE_MAX_PCT = exports.CASH_IN_QUOTE_MIN_PCT = void 0;
exports.isMonthKey = isMonthKey;
exports.monthIndex = monthIndex;
exports.normalizeRevenueCalibrationMode = normalizeRevenueCalibrationMode;
exports.normalizeCalibrationHorizonMonths = normalizeCalibrationHorizonMonths;
exports.computeCalibrationFactor = computeCalibrationFactor;
exports.clampPct = clampPct;
exports.parsePayoutPctInput = parsePayoutPctInput;
exports.buildHistoricalPayoutPrior = buildHistoricalPayoutPrior;
exports.buildPayoutRecommendation = buildPayoutRecommendation;
exports.normalizeRevenueCalibrationState = normalizeRevenueCalibrationState;
exports.learnRevenueCalibrationState = learnRevenueCalibrationState;
exports.buildCalibrationProfile = buildCalibrationProfile;
const CASH_IN_CALIBRATION_HORIZON_OPTIONS = [3, 6, 12];
exports.CASH_IN_QUOTE_MIN_PCT = 40;
exports.CASH_IN_QUOTE_MAX_PCT = 60;
exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT = 51;
exports.CASH_IN_SEASONALITY_CAP_PCT = 8;
exports.CASH_IN_RISK_CAP_PCT = 6;
exports.CASH_IN_SHRINKAGE_MIN_SAMPLES = 3;
exports.CASH_IN_PLAN_SAFETY_MARGIN_PCT = 0.3;
exports.CASH_IN_SEASONALITY_PROFILE_SOURCE_LABEL = "Historie (Mai 2022–Jan 2026)";
exports.CASH_IN_SEASONALITY_PRIOR_DEFAULT_BY_MONTH = Object.freeze([
    -0.84,
    0.71,
    2.08,
    -1.65,
...
exports.REVENUE_CALIBRATION_ALPHA = 0.20;
exports.REVENUE_CALIBRATION_GAMMA = 0.25;
exports.REVENUE_CALIBRATION_BIAS_MIN = 0.75;
exports.REVENUE_CALIBRATION_BIAS_MAX = 1.05;
exports.REVENUE_CALIBRATION_CONSERVATIVE_MIN = 0.70;
exports.REVENUE_CALIBRATION_CONSERVATIVE_MAX = 1.00;
exports.REVENUE_CALIBRATION_RISK_MAX = 0.10;
exports.REVENUE_CALIBRATION_DEFAULT_BIAS = 1.00;
exports.REVENUE_CALIBRATION_DEFAULT_RISK = 0.05;
exports.REVENUE_CALIBRATION_LIVE_FACTOR_MIN = 0.60;
exports.REVENUE_CALIBRATION_LIVE_FACTOR_MAX = 1.20;
exports.REVENUE_CALIBRATION_LIVE_HORIZON_MONTHS = 3;
...
function normalizeRevenueCalibrationMode(value) {
    return String(value || "").trim().toLowerCase() === "conservative"
        ? "conservative"
        : "basis";
}
function normalizeCalibrationHorizonMonths(value, fallback = 6) {
    const numeric = Math.round(Number(value || 0));
    if (CASH_IN_CALIBRATION_HORIZON_OPTIONS.includes(numeric))
        return numeric;
...
function computeCalibrationFactor(rawFactor, horizonMonths, offsetMonths) {
...
function clampPct(value, minPct = exports.CASH_IN_QUOTE_MIN_PCT, maxPct = exports.CASH_IN_QUOTE_MAX_PCT) {
    const numeric = Number(value);
    if (!Number.isFinite(numeric))
        return minPct;
    return Math.min(maxPct, Math.max(minPct, numeric));
}
function parsePayoutPctInput(value) {
...
function clampSeasonalityPct(value) {
    ...
    if (!Number.isFinite(numeric))
        return 0;
    return Math.min(exports.CASH_IN_SEASONALITY_CAP_PCT, Math.max(-exports.CASH_IN_SEASONALITY_CAP_PCT, numeric));
}
...
function normalizeSeasonalityOffsetsToZero(values) {
    const base = Array.from({ length: 12 }, (_, index) => clampSeasonalityPct(Array.isArray(values) ? values[index] : 0));
    ...
    for (let idx = centered.length - 1; idx >= 0; idx -= 1) {
        ...
        if (candidate < -exports.CASH_IN_SEASONALITY_CAP_PCT || candidate > exports.CASH_IN_SEASONALITY_CAP_PCT)
            continue;
...
function normalizePredictionSnapshotMap(raw) {
    ...
    Object.entries(raw).forEach(([month, value]) => {
        ...
        if (Number.isFinite(Number(value))) {
            out[month] = {
                quotePct: clampPct(Number(value), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT),
                mode: "legacy",
                source: "legacy",
                createdAt: null,
            };
        ...
        if (!Number.isFinite(quoteRaw))
            return;
        out[month] = {
            quotePct: clampPct(Number(quoteRaw), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT),
...
function normalizeHistoricalImport(raw) {
    ...
    const levelPct = Number.isFinite(levelRaw)
        ? clampPct(Number(levelRaw), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
        : null;
    const seasonalityPriorByMonth = toSeasonalityArray(raw.seasonalityPriorByMonth ?? raw.seasonalityByMonth, 0);
...
function normalizeLearningState(raw, options = {}) {
    ...
    const fallbackLevelRaw = parsePayoutPctInput(options.fallbackLevelPct ?? source.baselineNormalPct ?? exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT);
    const fallbackLevelPct = Number.isFinite(fallbackLevelRaw)
        ? clampPct(Number(fallbackLevelRaw), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
        : exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT;
    const importedProfile = normalizeHistoricalImport(source.historicalImport || source.importProfile);
    const fallbackPriorOffsets = normalizeSeasonalityOffsetsToZero(exports.CASH_IN_SEASONALITY_PRIOR_DEFAULT_BY_MONTH);
    const importedPriorOffsets = importedProfile
        ? normalizeSeasonalityOffsetsToZero(toSeasonalityArray(importedProfile.seasonalityPriorByMonth, 0))
        : fallbackPriorOffsets.slice();
    const priorBase = importedPriorOffsets.length === 12
        ? importedPriorOffsets
        : fallbackPriorOffsets;
    const rawPriorOffsets = normalizeSeasonalityOffsetsToZero(toSeasonalityArray(source.seasonalityPriorByMonth, 0));
    ...
    const rawSeasonalityOffsets = normalizeSeasonalityOffsetsToZero(toSeasonalityArray(source.seasonalityByMonth, 0));
    ...
    const riskBasePct = Number.isFinite(riskBaseRaw)
        ? Math.min(exports.CASH_IN_RISK_CAP_PCT, Math.max(0, Number(riskBaseRaw)))
        : 0;
    return {
        levelPct,
        seasonalityByMonth,
        ...
        predictionSnapshotByMonth: normalizePredictionSnapshotMap(source.predictionSnapshotByMonth || source.recommendationSnapshotByMonth),
        importedProfile,
    };
}
function serializeLearningState(state, input = {}) {
    ...
    return {
        version: 1,
        levelPct: clampPct(Number(state.levelPct || exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT)),
        ...
        seasonalitySampleCountByMonth: buildMonthMapFromArray(state.seasonalitySampleCountByMonth, (value) => (Math.max(0, Math.round(Number(value || 0))))),
        riskBasePct: Math.min(exports.CASH_IN_RISK_CAP_PCT, Math.max(0, Number(state.riskBasePct || 0))),
        ...
        historicalImport: state.importedProfile
            ? {
                ...
                levelPct: Number.isFinite(Number(state.importedProfile.levelPct))
                    ...
                    : null,
                seasonalityPriorByMonth: buildMonthMapFromArray(state.importedProfile.seasonalityPriorByMonth, (value) => clampSeasonalityPct(value)),
                monthSampleCountByMonth: buildMonthMapFromArray(state.importedProfile.monthSampleCountByMonth, (value) => Math.max(0, Math.round(Number(value || 0)))),
        ...
        config: {
            ...
            riskCapPct: exports.CASH_IN_RISK_CAP_PCT,
            seasonalityCapPct: exports.CASH_IN_SEASONALITY_CAP_PCT,
            shrinkageMinSamples: exports.CASH_IN_SHRINKAGE_MIN_SAMPLES,
...
function computeMonthRiskAdjustmentPct(riskBasePct, horizonMonths) {
    ...
    return Math.min(exports.CASH_IN_RISK_CAP_PCT, scaled);
}
function computeSeasonalityWithShrinkage({ slotIndex, seasonalityByMonth, seasonalityPriorByMonth, seasonalitySampleCountByMonth, enabled, }) {
    if (!enabled) {
        return {
            valuePct: 0,
    ...
    const weight = Math.min(1, sampleCount / exports.CASH_IN_SHRINKAGE_MIN_SAMPLES);
...
function buildHistoricalPayoutPrior(input = {}) {
    ...
    const normalizedValues = valuesSource
        ...
        .filter((value) => Number.isFinite(value))
        .map((value) => clampPct(Number(value), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT));
    ...
    const robustValues = normalizedValues.filter((value) => {
        if (!(Number.isFinite(outlierLow) && Number.isFinite(outlierHigh)))
            return true;
    ...
    const levelPct = Number.isFinite(levelMedian)
        ? clampPct(Number(levelMedian), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
        : exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT;
    ...
    const seasonalityPriorByMonth = monthlyResiduals.map((residuals) => {
        ...
        if (!Number.isFinite(residualMedian))
            return 0;
...
function resolveLiveSignal(input = {}) {
    ...
    if (!(dayOfMonth >= CASH_IN_LIVE_SIGNAL_START_DAY))
        return null;
    const incomings = Array.isArray(input.incomings) ? input.incomings : [];
    ...
    if (!Number.isFinite(quoteRaw))
        return null;
    const quotePct = clampPct(quoteRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT);
...
function computeWeightedLevelForMonth({ points, referenceMonth, fallbackLevelPct, }) {
    const fallback = clampPct(Number.isFinite(Number(fallbackLevelPct))
        ? Number(fallbackLevelPct)
        : exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT);
    ...
    (Array.isArray(points) ? points : []).forEach((point) => {
        ...
        if (ageMonths < 0 || ageMonths >= CASH_IN_LEVEL_WINDOW_MONTHS)
            return;
        ...
        samples.push({
            ...
            quotePct: clampPct(quotePct, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT),
        });
    });
    ...
    const levelRaw = Number.isFinite(avg3Pct) && Number.isFinite(avg12Pct)
        ...
        : Number.isFinite(avg3Pct)
            ...
            : Number.isFinite(avg12Pct)
                ? Number(avg12Pct)
                : fallback;
    const levelPct = clampPct(levelRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT);
...
function computeSeasonalityProfile({ points, currentMonth, enabled, priorOffsets, priorSampleCounts, }) {
    ...
    eligiblePoints.forEach((point) => {
        const pointMonthIndex = monthIndex(point?.month);
        const slotIndex = toMonthSlotIndex(point?.month);
        if (pointMonthIndex == null || slotIndex == null)
            return;
        const quotePct = Number(point?.quotePct);
        if (!Number.isFinite(quotePct))
            return;
        monthlyQuotes[slotIndex].push(clampPct(quotePct, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT));
...
function buildPayoutRecommendation(input = {}) {
    const monthlyActuals = input.monthlyActuals && typeof input.monthlyActuals === "object"
        ? input.monthlyActuals
        : {};
    const incomings = Array.isArray(input.incomings) ? input.incomings : [];
    ...
    const istPoints = Object.entries(monthlyActuals)
        ...
        if (!Number.isFinite(quotePctRaw))
            return null;
        return {
            month,
            quotePct: clampPct(Number(quotePctRaw), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT),
    ...
    const fallbackLevelPct = Number.isFinite(baselineFallbackRaw)
        ? clampPct(Number(baselineFallbackRaw), exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
        : clampPct(Number(learningState.levelPct || exports.CASH_IN_BASELINE_NORMAL_DEFAULT_PCT));
    ...
    const fixedSafetyMarginPct = exports.CASH_IN_PLAN_SAFETY_MARGIN_PCT;
    ...
    months.forEach((month) => {
        ...
        if (istPoint) {
            byMonth[month] = {
                month,
                quotePct: clampPct(istPoint.quotePct, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT),
                sourceTag: "IST",
                ...
                seasonalityProfileSource: exports.CASH_IN_SEASONALITY_PROFILE_SOURCE_LABEL,
                shrinkageActive: false,
                capsApplied: [],
                capApplied: false,
        ...
        const quotePct = clampPct(quoteBeforeClampPct, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT);
        appliedModelPoints.push({ month, quotePct });
        const capsApplied = [];
        if (Math.abs(quoteBeforeClampPct - quotePct) > 0.000001) {
            capsApplied.push("quote_band_40_60");
        }
        if (Math.abs(seasonalityRawPct) >= exports.CASH_IN_SEASONALITY_CAP_PCT - 0.000001) {
    ...
    return {
        ...
        observedNormalMedianPct: Number.isFinite(normalObservedMedianRaw)
            ? clampPct(normalObservedMedianRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
            : null,
        observedNormalAveragePct: Number.isFinite(normalObservedAverageRaw)
            ? clampPct(normalObservedAverageRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
            : null,
        ...
        observedNormalWithForecastMedianPct: Number.isFinite(modelObservedMedianRaw)
            ? clampPct(modelObservedMedianRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
            : null,
        observedNormalWithForecastAveragePct: Number.isFinite(modelObservedAverageRaw)
            ? clampPct(modelObservedAverageRaw, exports.CASH_IN_QUOTE_MIN_PCT, exports.CASH_IN_QUOTE_MAX_PCT)
            : null,
        ...
        seasonalitySampleCountByMonth: buildMonthMapFromArray(seasonalityProfile.sampleCounts, (value) => value),
...
function normalizeRevenueCalibrationForecastLock(raw) {
    ...
    Object.entries(raw).forEach(([month, lockRaw]) => {
        ...
        if (!Number.isFinite(forecastRevenueLockedEUR))
            return;
        out[month] = {
...
function normalizeRevenueCalibrationState(raw) {
    const source = raw && typeof raw === "object" ? raw : {};
    return {
        biasB: clampNumber(source.biasB, exports.REVENUE_CALIBRATION_BIAS_MIN, exports.REVENUE_CALIBRATION_BIAS_MAX, exports.REVENUE_CALIBRATION_DEFAULT_BIAS),
        riskR: clampNumber(source.riskR, 0, exports.REVENUE_CALIBRATION_RISK_MAX, exports.REVENUE_CALIBRATION_DEFAULT_RISK),
...
function computeLiveAnchor({ incomings, currentMonth, forecastRevenueByMonth }) {
    ...
    const cLive = clampNumber(cLiveRaw, exports.REVENUE_CALIBRATION_LIVE_FACTOR_MIN, exports.REVENUE_CALIBRATION_LIVE_FACTOR_MAX, exports.REVENUE_CALIBRATION_DEFAULT_BIAS);
    return {
        enabled: true,
        reason: "ok",
        f0: currentForecastRevenue,
...
function isPastMonth(month, currentMonth) {
    if (!isMonthKey(month) || !isMonthKey(currentMonth))
        return false;
    return month < currentMonth;
}
function learnRevenueCalibrationState(input = {}) {
    const now = input.now instanceof Date && !Number.isNaN(input.now.getTime())
        ? input.now
        : new Date();
    ...
    if (learnablePoints.length) {
        biasB = exports.REVENUE_CALIBRATION_DEFAULT_BIAS;
        riskR = exports.REVENUE_CALIBRATION_DEFAULT_RISK;
        learnablePoints.forEach((point) => {
            const accuracy = Number(point.actualRevenue) / Number(point.lockedRevenue);
            const biasBefore = biasB;
            const biasAfter = clampNumber(((1 - exports.REVENUE_CALIBRATION_ALPHA) * biasBefore) + (exports.REVENUE_CALIBRATION_ALPHA * accuracy), exports.REVENUE_CALIBRATION_BIAS_MIN, exports.REVENUE_CALIBRATION_BIAS_MAX, biasBefore);
            const optimismGap = Math.max(0, biasBefore - accuracy);
            const riskAfterRaw = ((1 - exports.REVENUE_CALIBRATION_GAMMA) * riskR) + (exports.REVENUE_CALIBRATION_GAMMA * optimismGap);
            const riskAfter = clampNumber(riskAfterRaw, 0, exports.REVENUE_CALIBRATION_RISK_MAX, riskR);
            replay.push({
                month: point.month,
...
function buildCalibrationProfile(input = {}) {
    const now = input.now instanceof Date && !Number.isNaN(input.now.getTime())
        ? input.now
        : new Date();
    ...
    const biasB = Number(learning.state?.biasB ?? exports.REVENUE_CALIBRATION_DEFAULT_BIAS);
    const riskR = Number(learning.state?.riskR ?? exports.REVENUE_CALIBRATION_DEFAULT_RISK);
    ...
    normalizedMonths.forEach((month) => {
        ...
        const wH = horizonOffsetRaw < 0
            ? 0
            : Math.max(0, 1 - (horizonOffset / exports.REVENUE_CALIBRATION_LIVE_HORIZON_MONTHS));
        const wEff = liveAnchor.enabled ? (wTime * wH) : 0;
        const signal = (wEff * Number(liveAnchor.cLive ?? biasB)) + ((1 - wEff) * biasB);
        const factorBasis = clampNumber(signal, exports.REVENUE_CALIBRATION_BIAS_MIN, exports.REVENUE_CALIBRATION_BIAS_MAX, 1);
        const riskScale = 1 + (0.1 * Math.min(horizonOffset, 4));
        const factorConservative = clampNumber(signal - (riskR * riskScale), exports.REVENUE_CALIBRATION_CONSERVATIVE_MIN, exports.REVENUE_CALIBRATION_CONSERVATIVE_MAX, 1);
    ...
    return {
        mode,
        horizonMonths: exports.REVENUE_CALIBRATION_LIVE_HORIZON_MONTHS,
        currentMonth,
        dayOfMonth,
        biasB,
        riskR,
