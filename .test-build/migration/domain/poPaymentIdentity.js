...
exports.normalizePoAutoEventType = normalizePoAutoEventType;
exports.inferPoAutoEventTypeFromId = inferPoAutoEventTypeFromId;
exports.getCanonicalPoAutoEventId = getCanonicalPoAutoEventId;
exports.isCanonicalPoAutoEventId = isCanonicalPoAutoEventId;
exports.hasExplicitPoPaymentEvidence = hasExplicitPoPaymentEvidence;
exports.normalizePoPaymentStateRecord = normalizePoPaymentStateRecord;
exports.backfillPoPaymentState = backfillPoPaymentState;
...
function mergeFilledObject(currentRaw, incomingRaw) {
    ...
    if (!incoming)
        return Object.keys(current).length ? current : incomingRaw;
...
function recordConflict(report, claim) {
    if (!claim || !claim.paymentId || !claim.legacyEventId)
        return;
...
function normalizePoAutoEventType(value) {
    const raw = normalizeText(value);
    if (!raw)
        return "";
    if (PO_AUTO_EVENT_TYPES.includes(raw))
        return raw;
...
function inferPoAutoEventTypeFromId(value) {
    const raw = normalizeText(value);
    if (!raw)
        return "";
    for (const type of PO_AUTO_EVENT_TYPES) {
        ...
        for (const alias of aliases) {
            if (raw === `auto-${alias}`)
                return type;
            if (raw.endsWith(`-auto-${alias}`))
                return type;
            if (raw.startsWith("po-auto-") && raw.endsWith(`-${alias}`))
                return type;
        }
    }
    return "";
}
function getCanonicalPoAutoEventId(record, type) {
...
function isCanonicalPoAutoEventId(record, eventId, type = "") {
...
function hasExplicitPoPaymentEvidence(entry) {
    const source = entry && typeof entry === "object" ? entry : {};
    if (isPaidLikeValue(source.status) || isPaidLikeValue(source.paid))
        return true;
    if (normalizeText(source.paidDate))
        return true;
    if (normalizeText(source.paymentId))
        return true;
...
function normalizePoPaymentStateRecord(input, options = {}) {
    ...
    PO_AUTO_EVENT_TYPES.forEach((type) => {
        ...
        getTypeAliases(type).forEach((alias) => {
            ...
            if (genericId !== canonicalId)
                aliasMap.set(genericId, canonicalId);
            if (ownerScopedId !== canonicalId)
                aliasMap.set(ownerScopedId, canonicalId);
        });
    });
    ...
    if (Array.isArray(record.paymentTransactions)) {
        record.paymentTransactions = record.paymentTransactions.map((entryRaw) => {
            ...
            if (!entry || !Array.isArray(entry.eventIds))
                return entry;
    ...
    if (changed)
        report.posRecordsChanged += 1;
    return { record, changed, aliasMap, canonicalIdsByType, report };
}
function backfillPoPaymentState(input, options = {}) {
    const mutate = options?.mutate === true;
        ...
        state.pos = state.pos.map((entryRaw) => {
            ...
            Object.entries(paymentLog).forEach(([eventId, logEntryRaw]) => {
                ...
                result.aliasMap.forEach((canonicalEventId, legacyEventId) => {
                    if (canonicalEventId !== eventId || legacyEventId === eventId)
                        return;
            ...
            if (Array.isArray(payment.coveredEventIds)) {
                ...
                payment.coveredEventIds.forEach((eventIdRaw) => {
                    ...
                    if (resolution?.type === "conflict")
                        recordConflict(report, resolution.claim);
                });
            ...
            if (Array.isArray(payment.allocations)) {
                payment.allocations = payment.allocations.map((allocationRaw) => {
                    ...
                    ["eventId", "plannedId"].forEach((field) => {
                        ...
                        if (!autoType || currentId.startsWith("po-auto-"))
                            return;
                        ...
                        if (resolution.canonicalEventId === currentId)
                            return;
                        allocation[field] = resolution.canonicalEventId;
                        if (field === "eventId")
                            report.paymentAllocationEventIdsRewritten += 1;
                        if (field === "plannedId")
                            report.paymentAllocationPlannedIdsRewritten += 1;
                    });
                    return allocation;
                });
            }
