...
exports.parseEuro = parseEuro;
exports.parsePct = parsePct;
...
function normalizeBucketScopeSet(input) {
    ...
    if (input instanceof Set)
        return input.size ? input : null;
    if (Array.isArray(input)) {
        const normalized = input
            .map((entry) => (0, portfolioBuckets_js_1.normalizePortfolioBucket)(entry, ''))
            .filter((entry) => portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.includes(entry));
...
function resolveCashInMeta(entry) {
    ...
    if (!meta || typeof meta.cashIn !== 'object')
        return null;
    return meta.cashIn;
}
function entryInBucketScope(entry, bucketScopeSet) {
    if (!bucketScopeSet || !bucketScopeSet.size)
        return true;
    const meta = (entry?.meta && typeof entry.meta === 'object') ? entry.meta : null;
    const rawBucket = entry?.portfolioBucket || meta?.portfolioBucket || '';
    const bucket = (0, portfolioBuckets_js_1.normalizePortfolioBucket)(rawBucket, portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE);
...
function seasonalitySourceLabel(tag) {
    ...
    if (!normalized)
        return '';
    if (normalized === 'ist_month')
        return 'Ist-Daten (Kalendermonat)';
    if (normalized === 'history_month')
        return 'Historie (Kalendermonat)';
...
function buildCashInTooltip({ forecastRevenue, calibrationFactorApplied, planRevenue, payoutPct, payoutAmount, quoteSource, recommendationSourceTag, recommendationExplanation, cashInMeta, }) {
            ...
            : recommendationSourceTag === 'RECOMMENDED_BASIS'
                ...
                : recommendationSourceTag === 'RECOMMENDED_CONSERVATIVE'
                    ...
                    : recommendationSourceTag === 'PROGNOSE'
                        ? 'Live-Signal'
                        : 'Empfehlung';
    const calibrationMode = (0, cashInRules_js_1.normalizeRevenueCalibrationMode)(cashInMeta?.calibrationMode);
    ...
    if (cashInMeta?.calibrationEnabled !== false) {
        ...
        parts.push(`C_live: ${formatTooltipFactor(cLive, 3)} · W_time: ${formatTooltipFactor(wTime, 3)} · W_h: ${formatTooltipFactor(wH, 3)} · W_eff: ${formatTooltipFactor(wEff, 3)} · d: ${dayOfMonth}`);
...
function buildActualMap(state) {
    ...
    Object.entries(monthlyActuals).forEach(([month, entry]) => {
        ...
        if (Number.isFinite(payoutEur))
            actual.payout = payoutEur;
        if (Number.isFinite(payoutRatePct))
            actual.payoutRatePct = payoutRatePct;
    ...
    actuals.forEach(entry => {
        ...
        if (!Number.isFinite(current.revenue) && Number.isFinite(revenue))
            current.revenue = revenue;
        if (!Number.isFinite(current.payout) && Number.isFinite(payout))
            current.payout = payout;
        if (!Number.isFinite(current.closing) && Number.isFinite(closing))
...
function buildOrderBucketSegments(input) {
    ...
    const fallbackBucket = input?.fallbackBucket || portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE;
    ...
    items.forEach((item) => {
        ...
        if (!sku)
            return;
        const key = (0, portfolioBuckets_js_1.normalizeSkuKey)(sku);
        const profile = profileBySku.get(key);
        const includeInForecast = profile
            ? profile.includeInForecast
            : (0, portfolioBuckets_js_1.normalizeIncludeInForecast)(undefined, true);
        if (!includeInForecast)
            return;
        const bucket = profile
            ? profile.effectivePortfolioBucket
            : (0, portfolioBuckets_js_1.resolveEffectivePortfolioBucket)({ sku, poSkuSet, fallbackBucket });
...
function computeOutflowStack(entries = []) {
    const stack = {
        fixedCosts: 0,
        poPaid: 0,
        poOpen: 0,
...
function evaluatePaidState({ statusRecord, autoEligible, autoManualCheck, entryTime, todayTime, defaultPaid }) {
    ...
    if (autoEligible) {
        if (autoApplied)
            autoTooltip = 'Automatisch bezahlt am Fälligkeitstag';
        else if (autoManualCheck)
            autoTooltip = 'Automatische Zahlung – manuelle Prüfung aktiv';
        else
            autoTooltip = 'Automatische Zahlung';
    }
    return {
        paid,
        autoApplied,
...
function expandFixcostInstances(state, opts = {}) {
    const s = state || {};
    ...
    fixcosts.forEach((master, idx) => {
        ...
        if (freq === 'quarterly')
            interval = 3;
        else if (freq === 'semiannual' || freq === 'halbjährlich')
            interval = 6;
        else if (freq === 'annual' || freq === 'jährlich' || freq === 'yearly')
            interval = 12;
        ...
        months.forEach(monthKey => {
            ...
            if (overrideApplied)
                tooltipParts.push('Override aktiv');
            if (prorationApplied)
                tooltipParts.push('Proratiert');
...
function computeSeries(state) {
    ...
    const { map: productProfileBySku, poSkuSet } = (0, portfolioBuckets_js_1.buildProductProfileIndex)(s);
    function profileForSku(sku) {
        const key = (0, portfolioBuckets_js_1.normalizeSkuKey)(sku);
        if (!key) {
            return {
                includeInForecast: true,
                effectivePortfolioBucket: portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE,
            };
        }
        return productProfileBySku.get(key) || {
            includeInForecast: true,
            effectivePortfolioBucket: (0, portfolioBuckets_js_1.resolveEffectivePortfolioBucket)({
                sku,
                poSkuSet,
                fallbackBucket: portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE,
            }),
        };
    }
    ...
    function baseEntry(overrides, meta = {}) {
        ...
        const portfolioBucket = overrides.portfolioBucket
            ? (0, portfolioBuckets_js_1.normalizePortfolioBucket)(overrides.portfolioBucket, portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE)
            : null;
        if (portfolioBucket) {
    ...
    const forecastMap = {};
    const forecastMapLiveByBucket = portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.reduce((acc, bucketName) => {
        acc[bucketName] = {};
        return acc;
    }, {});
    const forecastMapPlanByBucket = portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.reduce((acc, bucketName) => {
        acc[bucketName] = {};
        return acc;
    }, {});
    ...
    if (forecastEnabled) {
        ...
        const planRevenueByMonth = (0, planProducts_js_1.buildPlanProductRevenueByMonthAndBucket)({
            state: s,
            months,
        });
        Object.entries(planRevenueByMonth?.byBucket || {}).forEach(([bucketNameRaw, monthMap]) => {
            const bucketName = (0, portfolioBuckets_js_1.normalizePortfolioBucket)(bucketNameRaw, portfolioBuckets_js_1.PORTFOLIO_BUCKET.PLAN);
    ...
    const cashInQuoteMinPct = cashInRules_js_1.CASH_IN_QUOTE_MIN_PCT;
    const cashInQuoteMaxPct = cashInRules_js_1.CASH_IN_QUOTE_MAX_PCT;
    ...
    const recommendationMedianRaw = (0, cashInRules_js_1.parsePayoutPctInput)(payoutRecommendation.observedNormalMedianPct);
    const recommendationMedianPct = Number.isFinite(recommendationMedianRaw)
        ? (0, cashInRules_js_1.clampPct)(recommendationMedianRaw, cashInQuoteMinPct, cashInQuoteMaxPct)
        : null;
    const recommendationBaselineNormalRaw = (0, cashInRules_js_1.parsePayoutPctInput)(payoutRecommendation.baselineNormalPct);
    const recommendationBaselineNormalPct = Number.isFinite(recommendationBaselineNormalRaw)
        ? (0, cashInRules_js_1.clampPct)(recommendationBaselineNormalRaw, cashInQuoteMinPct, cashInQuoteMaxPct)
        : 51;
    ...
    const cashInCalibrationMode = (0, cashInRules_js_1.normalizeRevenueCalibrationMode)(s?.settings?.cashInCalibrationMode);
    const calibrationProfile = (0, cashInRules_js_1.buildCalibrationProfile)({
        incomings,
        months: Object.keys(bucket),
    ...
    const cashInCalibrationHorizonMonths = Math.max(1, Math.round(Number(calibrationProfile.horizonMonths || 3)));
    ...
    Object.keys(bucket).forEach(m => {
        ...
        const recommendationSourceTag = String(recommendationByMonth?.sourceTag
            || 'RECOMMENDED_PLAN');
        const recommendationExplanation = String(recommendationByMonth?.explanation || '').trim() || null;
        const recommendationQuoteRaw = (0, cashInRules_js_1.parsePayoutPctInput)(recommendationByMonth?.quotePct);
        const recommendationQuotePct = Number.isFinite(recommendationQuoteRaw)
            ? (0, cashInRules_js_1.clampPct)(recommendationQuoteRaw, cashInQuoteMinPct, cashInQuoteMaxPct)
        ...
        const basePayoutPct = (0, cashInRules_js_1.clampPct)(basePayoutPctRaw, cashInQuoteMinPct, cashInQuoteMaxPct);
        ...
        const calibrationFactorRaw = Number(calibration.factor
        ...
        const pushSalesEntry = ({ id, label, source, sourceTab, component, componentRevenueRaw, portfolioBucket, componentRevenue, }) => {
        ...
        if (hasCashInSetupRevenueOverride) {
            ...
            if (totalWeight > 0) {
                ...
                salesComponents.push({
                    bucketName: portfolioBuckets_js_1.PORTFOLIO_BUCKET.CORE,
                    source: 'sales',
                    sourceTab: '#eingaben',
    ...
    (Array.isArray(s.products) ? s.products : []).forEach((entry, productIndex) => {
        ...
        const launchCosts = (0, portfolioBuckets_js_1.normalizeLaunchCosts)(product.launchCosts, `prod-${productIndex + 1}-lc`);
        launchCosts.forEach((cost, index) => {
            const month = String(cost.date || '').slice(0, 7);
            if (!month || !bucket[month])
                return;
            pushEntry(month, baseEntry({
                id: `launch-product-${(0, portfolioBuckets_js_1.normalizeSkuKey)(sku)}-${cost.id || index + 1}`,
                direction: 'out',
    ...
    (Array.isArray(s.planProducts) ? s.planProducts : []).forEach((entry, planIndex) => {
        ...
        if (status !== 'active')
            return;
        const includeInForecast = (0, portfolioBuckets_js_1.normalizeIncludeInForecast)(row.includeInForecast, true);
        if (!includeInForecast)
            return;
        const planSku = String(row.plannedSku || row.mappedSku || '').trim();
        const bucketName = (0, portfolioBuckets_js_1.resolveEffectivePortfolioBucket)({
            product: row,
            sku: planSku,
            poSkuSet,
        ...
        const launchCosts = (0, portfolioBuckets_js_1.normalizeLaunchCosts)(row.launchCosts, `plan-${planIndex + 1}-lc`);
        launchCosts.forEach((cost, index) => {
            const month = String(cost.date || '').slice(0, 7);
            if (!month || !bucket[month])
                return;
            pushEntry(month, baseEntry({
                id: `launch-plan-${(0, portfolioBuckets_js_1.normalizeSkuKey)(planSku || alias)}-${cost.id || index + 1}`,
                direction: 'out',
                amount: Math.abs(parseEuro(cost.amountEur)),
                label: `${alias} · Launch-Kosten (${cost.type || 'Sonstiges'})`,
                month,
            ...
            resolvedPoPayments.forEach((milestone) => {
                (Array.isArray(milestone.segments) ? milestone.segments : []).forEach((portion) => {
                    ...
                    pushEntry(m, {
                        ...
                        label: portion.label,
                        month: m,
                        date: portion.displayDate || portion.paidDate || portion.dueDate || null,
                        kind: portion.kind,
                        group: portion.group,
                        ...
                        meta: {
                            ...
                            poPaymentDisplayDate: portion.displayDate || null,
                            poPaymentDisplayDateKind: portion.displayDateKind || null,
                            poPaymentDueDate: portion.dueDate || null,
                            poPaymentPaidDate: portion.paidDate || null,
                            poPaymentTypeLabel: portion.typeLabel || null,
                            poPaymentDisplayAmountEur: Number(portion.amountEur || 0),
    ...
    (Array.isArray(s.fos) ? s.fos : []).forEach(fo => {
        ...
        segments.forEach((segment) => {
            expandOrderEvents(segment.row, settingsNorm, 'FO', 'foNo').forEach(ev => {
                ...
                pushEntry(m, baseEntry({
                    id: splitByBucket
                        ? `${ev.id || `fo-${fo.id || ''}-${ev.type}-${ev.month}`}-${(0, portfolioBuckets_js_1.normalizeSkuKey)(segment.bucket)}`
    ...
    const series = months.map(m => {
        ...
        entries.forEach((entry) => {
            const bucketName = (0, portfolioBuckets_js_1.normalizePortfolioBucket)(entry?.portfolioBucket || entry?.meta?.portfolioBucket, "");
            if (!bucketName || !portfolioBuckets_js_1.PORTFOLIO_BUCKET_VALUES.includes(bucketName))
                return;
...
function getEffectiveCashInMonth(monthKey, state, globalSettings = null, options = {}) {
    ...
    const payoutEUR = salesEntries.reduce((sum, entry) => {
        if (!entryInBucketScope(entry, bucketScopeSet))
            return sum;
        const amount = Math.abs(Number(entry?.amount || 0));
        if (!Number.isFinite(amount) || amount <= 0)
            return sum;
...
function buildEffectiveCashInByMonth(monthKeys, state, globalSettings = null, options = {}) {
