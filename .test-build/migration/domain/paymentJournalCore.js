...
exports.buildPaymentJournalRowsCore = buildPaymentJournalRowsCore;
...
function classifyPaymentPositions({ label, eventType }) {
    ...
    if (type === "fx_fee" || lowered.includes("fx"))
        return [];
    const set = new Set();
    if (type === "freight")
        set.add("Shipping");
    ...
    if (type === "eust")
        set.add("EUSt");
    if (type === "vat_refund" || type === "eust_refund")
        set.add("EUSt-Erstattung");
    if (lowered.includes("deposit") || lowered.includes("anzahlung"))
        set.add("Deposit");
    ...
    if (lowered.includes("shipping") || lowered.includes("fracht"))
        set.add("Shipping");
    if (lowered.includes("eust"))
        set.add("EUSt");
    if (lowered.includes("zoll") || lowered.includes("custom") || lowered.includes("duty"))
        set.add("Zoll");
    if (lowered.includes("refund") && lowered.includes("eust"))
        set.add("EUSt-Erstattung");
...
function resolveActualAmountForLine({ status, plannedEur, paymentRow, paymentRows, paymentRecord, paymentIndexes, paymentLogEntry, state, eventId, paidDate, }) {
    const issues = [];
    if (status !== "PAID")
    ...
    if (!paymentRow?.paymentId && paidDate && eventId) {
        const fallbackMatch = (Array.isArray(state?.payments) ? state.payments : []).find((entry) => {
            ...
            if (normalizeIsoDate(payment.paidDate) !== normalizeIsoDate(paidDate))
                return false;
            if (Array.isArray(payment.coveredEventIds) && payment.coveredEventIds.includes(eventId))
                return true;
            return false;
        });
...
function collectEventRowsForPo({ state, settings, supplierNameMap, skuAliasMap, paymentIndexes }) {
    ...
    poRecords.forEach((entry) => {
        ...
        const paymentRows = (0, orderEditorFactory_js_1.buildPaymentRows)(cloneRecord(record), PO_CONFIG, settings, payments, { includeIncoming: true });
        ...
        paymentRows.forEach((paymentRowRaw) => {
            ...
            if (!hasRelevantPosition(positions))
                return;
            const paymentId = firstNonEmpty(paymentRow.paymentId, logEntry.paymentId, paymentIndexes.paymentIdsByEvent.get(eventId));
            const paymentRecord = paymentId ? paymentIndexes.byId.get(paymentId) || null : null;
            const status = (isPaidLike(paymentRow.status)
...
function mergeGroupedPoPayments(eventRows, paymentIndexes) {
    ...
    eventRows.forEach((row) => {
        if (row.status === "PAID" && row.paymentId) {
            if (!groupedByPayment.has(row.paymentId))
                groupedByPayment.set(row.paymentId, []);
    ...
    groupedByPayment.forEach((rows, paymentId) => {
        ...
        let paidDate = normalizeIsoDate(firstNonEmpty(paymentRecord?.paidDate, ...rows.map((row) => row.paidDate)));
...
function collectFoRows({ state, supplierNameMap, skuAliasMap }) {
    ...
    foRecords.forEach((entry) => {
        ...
        record.payments.forEach((paymentRaw) => {
            ...
            if (String(payment.category || "").toLowerCase() === "eust_refund")
                return;
            const positions = classifyPaymentPositions({
...
function rowSortDate(row) {
    if (row.status === "PAID")
        return normalizeIsoDate(row.paidDate || row.dueDate);
...
function buildPaymentJournalRowsCore(input = {}) {
