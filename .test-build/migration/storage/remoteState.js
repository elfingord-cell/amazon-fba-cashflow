...
exports.ConfigurationError = exports.AuthRequiredError = exports.ConflictError = void 0;
exports.fetchRemoteState = fetchRemoteState;
exports.pushRemoteState = pushRemoteState;
const authSession_js_1 = require("./authSession.js");
...
async function buildDbHeaders() {
    ...
    if (!(0, authSession_js_1.isSupabaseConfigured)()) {
        throw new ConfigurationError("Supabase client config is missing.");
    }
    const token = await (0, authSession_js_1.getAccessToken)();
    if (!token) {
...
function appendWorkspaceArg(args = {}) {
    const workspaceId = (0, authSession_js_1.getWorkspaceId)();
    if (!workspaceId)
        return args;
...
async function fetchRemoteState() {
    ...
    try {
        const payload = await (0, supabaseApi_js_1.supabaseRpc)("app_bootstrap_client", appendWorkspaceArg(), { accessToken: token });
...
async function pushRemoteState({ ifMatchRev, updatedBy, data }) {
    const token = await buildDbHeaders();
    try {
        const payload = await (0, supabaseApi_js_1.supabaseRpc)("app_sync_client", appendWorkspaceArg({
            p_if_match_rev: ifMatchRev ?? null,
            p_state: data,
        }), { accessToken: token });
        const result = (0, supabaseApi_js_1.normalizeRpcPayload)(payload) || {};
        if (!result.ok) {
