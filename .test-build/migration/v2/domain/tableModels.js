...
function buildForecastProducts(state, categoriesById) {
    ...
    const forecastImportMap = (state.forecast && typeof state.forecast === "object"
        ...
        && typeof state.forecast.forecastImport === "object")
        ? state.forecast.forecastImport
        : {};
    ...
    const maskPlanByLive = (sku, planned) => {
        const out = {};
        Object.entries(planned || {}).forEach(([month, units]) => {
            const liveUnits = getImportValue(forecastImportMap, sku, month)?.units;
            if (Number.isFinite(liveUnits) && Number(liveUnits) > 0)
                return; // Live übernimmt diesen Monat
            out[month] = units;
        });
        return out;
    };
