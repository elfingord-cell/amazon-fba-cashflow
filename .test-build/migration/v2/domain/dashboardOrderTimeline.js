...
function resolvePoRecord(input) {
    ...
    const rows = Array.isArray(input.state.pos) ? input.state.pos : [];
    return rows.find((entry) => {
        const byId = normalizeRef(entry.id);
        const byNo = normalizeRef(entry.poNo);
        if (sourceId && sourceId === byId)
            return true;
        if (sourceNumber && sourceNumber === byNo)
            return true;
        if (sourceNumber && sourceNumber === byId)
            return true;
        return false;
    }) || null;
}
function resolveFoRecord(input) {
    const sourceId = normalizeRef(input.sourceId);
    const sourceNumber = normalizeRef(input.sourceNumber);
    const rows = Array.isArray(input.state.fos) ? input.state.fos : [];
    return rows.find((entry) => {
        const byId = normalizeRef(entry.id);
        ...
        if (sourceId && sourceId === byId)
            return true;
        if (sourceNumber && (sourceNumber === byFoNo || sourceNumber === byFoNumber || sourceNumber === byId))
            return true;
        return false;
    }) || null;
}
function resolvePaymentStatus(value, paidDate, paymentId) {
...
function resolvePaymentDirection(input) {
    ...
    if (text.includes("refund") || text.includes("erstatt"))
        return "in";
...
function buildFoPayments(foRecord) {
    const fxRate = Number(foRecord.fxRate || 0);
    const payments = Array.isArray(foRecord.payments) ? foRecord.payments : [];
    return payments
        .map((row, index) => {
        const dueDate = normalizeIsoDate(row.dueDate);
        if (!dueDate)
        ...
        const amountEur = Math.abs((0, orderUtils_1.convertToEur)(amountRaw, row.currency || "EUR", fxRate));
        if (!(amountEur > 0))
            return null;
        return {
...
function buildTimeline(input) {
    ...
    const timelinePoints = items.flatMap((entry) => [entry.startMs, entry.endMs].filter((value) => Number.isFinite(value)));
...
function buildDashboardOrderTimeline(input) {
    ...
    if (source === "po") {
        ...
        const settings = (input.state.settings && typeof input.state.settings === "object")
            ...
            : {};
        const aggregate = (0, orderUtils_1.computePoAggregateMetrics)({
            items: poRecord.items,
            orderDate: poRecord.orderDate,
            fxRate: poRecord.fxOverride ?? settings.fxRate,
            fallback: poRecord,
        });
        const schedule = (0, orderUtils_1.computeScheduleFromOrderDate)({
            orderDate: poRecord.orderDate,
    ...
    if (!foRecord)
        return null;
    const foSchedule = (0, orderUtils_1.computeFoSchedule)({
