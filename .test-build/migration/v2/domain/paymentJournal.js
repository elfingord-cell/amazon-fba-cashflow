...
exports.buildPaymentJournalRowsFromState = buildPaymentJournalRowsFromState;
exports.buildPaymentJournalCsvRows = buildPaymentJournalCsvRows;
exports.paymentJournalRowsToCsv = paymentJournalRowsToCsv;
exports.sumPaymentRows = sumPaymentRows;
exports.openPaymentJournalPrintView = openPaymentJournalPrintView;
...
function normalizePaymentType(input) {
    ...
    if (eventType === "fx_fee" || label.includes("fx"))
        return null;
    if (eventType === "freight" || label.includes("shipping") || label.includes("fracht"))
        return "Fracht";
    if (eventType === "eust" || label.includes("eust"))
        return "EUSt";
    if (label.includes("balance2") || label.includes("balance 2") || label.includes("second balance"))
        return "Balance2";
...
function toOrderSettings(state) {
    ...
    return {
        ...
        cny: raw.cny && typeof raw.cny === "object"
            ? {
                start: String(raw.cny.start || ""),
                end: String(raw.cny.end || ""),
            }
            : { start: "", end: "" },
...
function resolveActualAllocation(input) {
    if (!input.payment || !input.paymentRow?.paymentId)
        return null;
    const total = Number(input.payment.amountActualEurTotal);
    if (!Number.isFinite(total))
        return null;
    const related = input.paymentRows.filter((row) => row.paymentId && row.paymentId === input.paymentRow.paymentId);
    if (!related.length)
        return null;
    const allocations = allocateByPlanned(total, related.map((row) => ({ id: String(row.id || ""), plannedEur: Number(row.plannedEur || 0) })));
    if (!allocations)
        return null;
...
function resolveActualAmountForLine(input) {
    const issues = [];
    if (input.row.status !== "PAID")
        return { amountActualEur: null, issues };
    if (!input.row.paidDate)
    ...
    if (input.paymentRecord?.allocations && Array.isArray(input.paymentRecord.allocations)) {
        const allocation = input.paymentRecord.allocations.find((entry) => {
            if (!entry)
                return false;
            if (entry.eventId && input.paymentRow?.id && entry.eventId === input.paymentRow.id)
                return true;
            if (entry.plannedId && input.paymentRow?.id && entry.plannedId === input.paymentRow.id)
                return true;
    ...
    if (!input.paymentRow?.paymentId && input.row.paidDate && input.paymentRow?.id) {
        const match = (Array.isArray(input.state.payments) ? input.state.payments : []).find((entry) => {
            ...
            if (!payment)
                return false;
            if (payment.paidDate !== input.row.paidDate)
                return false;
            if (Array.isArray(payment.coveredEventIds) && payment.coveredEventIds.includes(input.paymentRow.id))
                return true;
            return false;
        });
...
function buildPaymentJournalRowsFromState(state, filters) {
    ...
    poRecords.forEach((entry) => {
        ...
        const paymentRows = (0, orderEditorFactory_js_1.buildPaymentRows)(snapshot, PO_CONFIG, settings, (Array.isArray(state.payments) ? state.payments : []));
    ...
    foRecords.forEach((entry) => {
        ...
        record.payments.forEach((payment) => {
            if (!payment)
                return;
            if (String(payment.category || "") === "eust_refund")
                return;
            ...
            const status = (String(payment.status || "").toUpperCase() === "PAID"
                || Boolean(paymentRecord)) ? "PAID" : "OPEN";
            const paidDate = String(payment.paidDate || paymentRecord?.paidDate || "");
            const actualFromPayment = Number(payment.paidEurActual);
            let amountActualEur = Number.isFinite(actualFromPayment) ? actualFromPayment : null;
            if (amountActualEur == null && paymentRecord?.allocations && Array.isArray(paymentRecord.allocations)) {
                const allocation = paymentRecord.allocations.find((entry) => String(entry?.eventId || "") === String(payment.id || ""));
    ...
    const filtered = rows.filter((row) => {
        if (scope === "paid" && row.status !== "PAID")
            return false;
        if (scope === "open" && row.status !== "OPEN")
            return false;
...
function buildPaymentJournalCsvRows(rows) {
    return rows.map((row) => ({
        month: row.month || "",
...
function paymentJournalRowsToCsv(rows, delimiter = ";") {
    const headers = [
        "month",
        "entityType",
        "poNumber",
...
function sumPaymentRows(rows, key) {
    return rows.reduce((sum, row) => sum + (Number(row[key]) || 0), 0);
}
function openPaymentJournalPrintView(rows, filters) {
