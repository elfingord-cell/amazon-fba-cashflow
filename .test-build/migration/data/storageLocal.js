...
exports.LAST_COMMIT_KEY = exports.STORAGE_KEY = void 0;
exports.createEmptyState = createEmptyState;
exports.loadState = loadState;
exports.commitState = commitState;
exports.saveState = saveState;
exports.getLastCommitInfo = getLastCommitInfo;
exports.addStateListener = addStateListener;
exports.exportState = exportState;
exports.importStateFile = importStateFile;
exports.getStatusSnapshot = getStatusSnapshot;
exports.setAutoManualCheck = setAutoManualCheck;
exports.setEventManualPaid = setEventManualPaid;
exports.clearEventManualPaid = clearEventManualPaid;
exports.setEventsManualPaid = setEventsManualPaid;
exports.setProductsTableColumns = setProductsTableColumns;
exports.getProductsSnapshot = getProductsSnapshot;
exports.getVatPreviewConfig = getVatPreviewConfig;
exports.updateVatPreviewSettings = updateVatPreviewSettings;
exports.updateVatPreviewMonth = updateVatPreviewMonth;
exports.resetVatPreviewMonths = resetVatPreviewMonths;
exports.getProductBySku = getProductBySku;
exports.upsertProduct = upsertProduct;
exports.upsertProductSupplier = upsertProductSupplier;
exports.deleteProductSupplier = deleteProductSupplier;
exports.setPreferredProductSupplier = setPreferredProductSupplier;
...
function parseEuro(value) {
    if (value == null)
        return 0;
    const parsed = (0, dataHealth_js_1.parseDeNumber)(String(value).replace(/€/g, ""));
...
function parseOptionalStoredNumber(value) {
    ...
    if (!raw)
        return null;
    const parsed = (0, dataHealth_js_1.parseDeNumber)(raw);
...
const defaults = {
    ...
    fixcosts: [],
    fixcostOverrides: {},
    taxes: (0, taxPlanner_js_1.createDefaultTaxesState)(),
    poTemplates: [],
    products: [],
    productCategories: [],
    recentProducts: [],
...
function ensureFixcostContainers(state) {
    if (!state)
        return;
    if (!Array.isArray(state.fixcosts))
        state.fixcosts = [];
...
function ensureTaxData(state) {
    if (!state)
        return;
    const base = (0, taxPlanner_js_1.createDefaultTaxesState)();
...
function ensurePoTemplates(state) {
    if (!state)
        return;
    if (!Array.isArray(state.poTemplates))
        state.poTemplates = [];
}
...
function ensureGlobalSettings(state) {
    ...
    if (!settings.productsTableColumns || typeof settings.productsTableColumns !== "object") {
        ...
        if (!Array.isArray(settings.productsTableColumns.list))
            settings.productsTableColumns.list = [];
        if (!Array.isArray(settings.productsTableColumns.grid))
            settings.productsTableColumns.grid = [];
    }
}
...
function ensureProductSuppliers(state) {
    if (!state)
        return;
    if (!Array.isArray(state.productSuppliers))
...
function ensureForecast(state) {
    ...
    if (!state.forecast.importSource)
        state.forecast.importSource = null;
    if (!Object.keys(state.forecast.forecastImport).length && state.forecast.items.length) {
        state.forecast.items.forEach(item => {
            ...
            if (isManual) {
                if (!state.forecast.forecastManual[skuKey])
                    state.forecast.forecastManual[skuKey] = {};
                ...
                if (!state.forecast.forecastImport[skuKey])
                    state.forecast.forecastImport[skuKey] = {};
...
function ensureMonthlyActuals(state) {
    ...
    Object.entries(state.monthlyActuals).forEach(([month, values]) => {
        ...
        const normalized = {};
        if (revenue != null)
            normalized.realRevenueEUR = revenue;
        if (payoutRate != null)
            normalized.realPayoutRatePct = payoutRate;
        if (closing != null)
            normalized.realClosingBalanceEUR = closing;
        if (realFixkosten != null)
            normalized.realFixkostenEUR = realFixkosten;
        if (Object.keys(normalized).length)
            cleaned[month] = normalized;
    });
...
function migrateLegacyOutgoings(state) {
    if (!state)
        return;
    if (!Array.isArray(state.outgoings) || !state.outgoings.length)
        return;
    ...
    rows.forEach(row => {
        ...
        if (!month)
            return;
        if (!state.fixcostOverrides[id][month])
            state.fixcostOverrides[id][month] = {};
...
function parseNumber(value) {
    return (0, dataHealth_js_1.parseDeNumber)(value);
}
function normaliseTemplate(template, options = {}) {
    ...
    if (template.name)
        next.name = String(template.name);
    if (template.supplierId)
        next.supplierId = String(template.supplierId);
...
function migrateProducts(state) {
    ...
    if (Array.isArray(state.pos))
        orders.push(...state.pos);
    if (Array.isArray(state.fos))
        orders.push(...state.fos);
...
function normaliseProductInput(input) {
    if (!input || typeof input !== "object")
        throw new Error("Produktdaten erforderlich");
    const sku = String(input.sku || "").trim();
    if (!sku)
        throw new Error("SKU darf nicht leer sein.");
...
function commitState(s, meta = {}) {
    ...
    try {
        localStorage.setItem(exports.LAST_COMMIT_KEY, lastCommitAt);
    }
    catch { }
...
function clearEventManualPaid(eventId) {
    if (!eventId)
        return;
    const state = loadState();
    const status = ensureStatusSection(state);
    const map = status.events;
    if (!map[eventId] || typeof map[eventId].manual === "undefined")
        return;
    delete map[eventId].manual;
    if (!Object.keys(map[eventId]).length)
        delete map[eventId];
    saveState(state, { source: "status:eventPaid", eventId });
}
function setEventsManualPaid(eventIds, paid) {
...
function setProductsTableColumns(view, widths) {
    if (!view || !Array.isArray(widths))
        return;
    const state = loadState();
...
function getVatPreviewConfig() {
    const state = loadState();
    ensureVatData(state);
    return { settings: state.settings.vatPreview, months: state.vatPreviewMonths };
}
function updateVatPreviewSettings(patch) {
    if (!patch || typeof patch !== "object")
        return;
    const state = loadState();
    ensureVatData(state);
    const target = state.settings.vatPreview;
    if (typeof patch.eustLagMonths !== "undefined")
        target.eustLagMonths = Number(patch.eustLagMonths) || target.eustLagMonths;
    if (typeof patch.deShareDefault !== "undefined")
        target.deShareDefault = Number(patch.deShareDefault);
    if (typeof patch.feeRateDefault !== "undefined")
        target.feeRateDefault = Number(patch.feeRateDefault);
    if (typeof patch.fixInputDefault !== "undefined")
        target.fixInputDefault = Number(patch.fixInputDefault);
    saveState(state, { source: "vat:settings" });
}
function updateVatPreviewMonth(month, patch) {
    if (!month || !patch || typeof patch !== "object")
        return;
    const state = loadState();
    ensureVatData(state);
    const target = state.vatPreviewMonths[month] || {};
    if (typeof patch.deShare !== "undefined")
        target.deShare = Number(patch.deShare);
    if (typeof patch.feeRateOfGross !== "undefined")
        target.feeRateOfGross = Number(patch.feeRateOfGross);
    if (typeof patch.fixInputVat !== "undefined")
        target.fixInputVat = Number(patch.fixInputVat);
    state.vatPreviewMonths[month] = target;
    saveState(state, { source: "vat:month", month });
}
function resetVatPreviewMonths() {
    const state = loadState();
    ensureVatData(state);
    state.vatPreviewMonths = {};
...
function upsertProductSupplier(input) {
    const state = loadState();
...
function setPreferredProductSupplier(id) {
    if (!id)
        return;
    const state = loadState();
    ensureProductSuppliers(state);
...
function setProductStatus(sku, status) {
    const state = loadState();
    ensureProducts(state);
    const key = productKey(sku);
...
function recordRecentProduct(sku) {
    const state = loadState();
    ensureProducts(state);
    const key = productKey(sku);
    if (!key)
